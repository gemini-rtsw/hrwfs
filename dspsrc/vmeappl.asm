...
Controller: SDSU2 
Revision: 0.25  (must agree with status word V_SW_ID in Y: memory)
...
CHK_VME	JSR	<VME_TST	; Test for incoming VMEbus commands
...
; Transmit frame status words to VME bus
	JSET	#OF_FRM,Y:<FLAGS,CLR_OF	; Skip if frame overflowed
...
CLR_OF	BCLR	#OF_FRM,Y:<FLAGS	; Clear overflow frame flag
	MOVE	#NUM_FSW,A 
	MOVE	R0,Y0
	ADD	Y0,A
	MOVE	A,R0
...
GET_FC	JSR	<CHK_FO		; Wait for datum
	JCS	<ABT_DON	; Timeout
	...
	MOVE	A,X0		; Temporary store of high byte
	JSR	<CHK_FO		; Wait for datum
	JCS	<ABT_DON	; Timeout
...
; Words 6 -> (3*NOUTS)+6 = pipeline garbage values
	...
	JSR	<CHK_FO		; Wait for datum
	JCS	<ABT_DON	; Timeout
	MOVE	X:RDFIFO,X0	; Get garbage pixel from the FIFO
...
RET_OF	JSET	#FBA_VLD,Y:<FLAGS,CHK_BUS	; Return if header already sent
	BSET	#OF_FRM,Y:<FLAGS	; Set overflow frame flag
...
; Discard buffered data
	MOVE	#INT_BUF,R1
	MOVE	R1,R6	; Clear internal buffer
	CLR	A
	MOVE	A,R7

; Discard rest of frame
DSC_FRM	MOVE	Y:<PIXELC,A
	TST	A
	JEQ	CHK_FC		; Skip if pixel counter is zero
	DO	A,L_ADSC
	JSR	<CHK_FO		; Wait for datum
	JCC	<RD_PXL		; Read datum if available
	ENDDO			; Timeout - exit loop
	JMP	<ABT_DON
RD_PXL	MOVE	X:RDFIFO,X0	; Get garbage pixel from the FIFO
L_ADSC

	JCLR	#EF,X:PCD,*	; Wait for the datum to be ready
	MOVE	X:RDFIFO,X0	; Get checksum from the FIFO
...
; Read header of next frame
	...
	DO	A0,L_GBGDSC
	JSR	<CHK_FO		; Wait for datum
	JCC	<RD_GBG		; Read datum if available
	ENDDO			; Timeout - exit loop
	JMP	<ABT_DON
...
CHK_FO	BCLR	#0,SR		; Initialize carry bit
	DO	Y:FO_TO,L_FTO	; Check FO_TO times for data 
	JCLR	#EF,X:PCD,FO_MT	; Wait for the datum to be ready
	ENDDO			; Abort loop
	RTS
FO_MT	NOP
L_FTO
	BSET	#0,SR		; Set carry bit to indicate timeout
	RTS
...
V_SW_ID	DC	$002500	; Software version number (00.25)


; *****  Readout parameters  *****

V_PSIZE	DC	$20	; packet size
...
PSIZE	DC	$20	; Packet size (in use copy)
...
OF_FRM	EQU	6	; Set to indicate an overflow in the next frame to
			; be transferred to the VME bus
...
HDR_MSK	DC	$FEFEF8	; Mask to check command header

FO_TO	DC	$FFFFFF	; Timeout: number of times to look for data in FIFO

; Constants for communication with timing board
