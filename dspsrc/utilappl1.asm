       COMMENT *
SDSU2 Utility Board Code
Instrument: Gemini WFS
Revision: 1.06   (must agree with U_SW_ID in Y: memory table)
...
2002/01/14 TDH - changes to make the code work as an EEPROM application
...
; Specify execution and load addresses.
	...
	ORG	P:APL_ADR,P:$1D00+APL_NUM*$300
	ENDIF
...
OSHUT	BCLR    #0,Y:U_DIG_OUT  ; Clear Port B bit #3 to open shutter
        BCLR    #ST_SH,X:<STATUS ; Clear status bit to mean shutter open
        RTS

CSHUT	BSET    #0,Y:U_DIG_OUT  ; Set Port B bit #3 to close shutter
        BSET    #ST_SH,X:<STATUS ; Set status to mean shutter closed
        RTS
...
;     because it contains application-specific commands
	...
	ELSE			; Memory offsets for generating EEPROMs
	ORG	X:COM_TBL,P:$1D00+APL_NUM*$300+($200-APL_ADR)
	ENDIF
...
U_SW_ID		DC	$010600	; Software version number 1.06
...
;   program has been downloaded. Here it is - 
	...
	ELSE			; Memory offsets for generating EEPROMs
	ORG	P:APL_ADR+1,P:$1D00+APL_NUM*$300+1
	ENDIF
