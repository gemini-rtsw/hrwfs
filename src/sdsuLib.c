static struct {void *v; char *c;} rcsid = {&rcsid,
	"$Id: sdsuLib.c,v 1.1.1.1 1999-03-17 03:14:23 cboyer Exp $"};
 ...
 *
 *	This library provides functions for sending commands to the SDSU controller,
 ...
 *	configuration files, plus various diagnostic functions, as listed below.
 *
 *	Further details of the SDSU controller hardware, firmware and DSP
 *	interface are contained in the "SDSU CCD Controller User's Manual",
 ...
 *	and hk4700 BSP sysLib libraries). However, sysextLib itself has proved to be
 *	highly non-portable and more trouble than it is worth, and it can been
 *	removed from this library by defining the NO_SYSEXTLIB macro when compiling it.
 *
 ...
 *	A single context structure is usually declared only once for each controller
 *	and a handle to the structure may subsequently be used by any tasks which
 *	make calls to sdsuLib.
 *
 ...
 *	sysextLib				- BSP system library extensions (only ifndef NO_SYSEXTLIB)
 ...
 *	sdsuBufferDelete		- destroy frame buffers previously allocated for readout
 ...
 *
 *	DEFICIENCIES:
 *	This library uses a polling method for detecting a reply from the SDSU controller,
 *	and the algorithm used can get confused if the controller is reset unexpectedly and
 *	the timing board generates an SYR. The algorithm can also get confused if some legitimate
 *	data just happens to contain ERR or SYR byte sequences.
 *
 *	Many of the timing parameters used by this library, such as the wait time while polling
 *	for a reply, the time to wait for a controller reset, and the timeouts used for the various
 *	commands, have been set arbitrarily.
 *
 *	I think the algorithm for reading out data from an SDSU controller, involving a message queue,
 *	an interrupt, various semaphores, a readout task and a signal handler, is more complicated
 *	than it needs to be. Having several threads makes handling errors difficult.
 *
 ...
 * Revision 1.53  1999/01/06 14:41:05  cics
 * 11 Dec 98 version provided to Corinne. Various comments corrected. List all frames on free list. Trap invalid readout task status. Do not allocate packet interrupt vector if not needed. Set frame aborted bit if aborted.
 *
 * Revision 1.49  1998/11/30 15:54:39  cics
 * Modifications made during SMB visit to Hilo, November 1998
 *
 * Revision 1.48  1998/10/22 15:14:43  cics
 * ICD TBD becomes ICD 1.6/1.10
 *
 * Revision 1.47  1998/10/21 16:32:08  cics
 * A bit of tidying up
 *
 * Revision 1.46  1998/10/21 10:43:42  cics
 * Removed SRA from sdsuContextCreate
 *
 * Revision 1.45  1998/10/20 13:45:36  cics
 * Tidy up sdsuContextDelete
 *
 * Revision 1.44  1998/10/20 09:28:30  cics
 * Fixed typo
 *
 * Revision 1.43  1998/10/14 09:48:50  cics
 * Read back test in sdsu_testWRM commented out
 *
 * Revision 1.42  1998/10/12 10:03:59  cics
 * Bug fix - Readout resources were not being freed in sdsuContextDelete
 *
 * Revision 1.41  1998/10/08 16:26:32  cics
 ...
 *
 * Revision 1.37  1998/09/17 08:27:16  cics
 * Worked around bug in sdsuFrameRelease. Added extra error checks to cache functions. Added more debug statements.
 *
 * Revision 1.36  1998/09/15 08:14:06  cics
 * Changed some constant parameters to const
 *
 * Revision 1.35  1998/09/09 14:35:32  cics
 * Global variables renamed to ensure they are unique
 *
 * Revision 1.34  1998/09/01 15:25:58  smb
 * anj removed control from data header and added messages to ISR functions
 *
 * Revision 1.33  1998/08/27 17:29:42  anj
 * Implemented frame & packet sync interrupts and application callbacks.
 *
 * Revision 1.32  1998/08/21 09:14:02  smb
 * Fixed typo. Sorry.
 *
 * Revision 1.30  1998/08/20 12:58:36  smb
 * Added sdsuParamUpload
 *
 * Revision 1.29  1998/08/20 12:21:04  anj
 * Modified the frame buffer allocation system.
 * Deleted sdsuSync, some mailbox stuff and other old Nick Dillon code
 * which will not be used.
 *
 * Revision 1.28  1998/08/18 13:52:43  smb
 * Tim Hardy's information included in sdsuPrintRepBuf
 *
 * Revision 1.27  1998/08/14 13:58:18  smb
 * Fixed bug in sdsuTest
 *
 * Revision 1.26  1998/08/13 09:11:20  smb
 * Added author comment
 *
 * Revision 1.25  1998/07/30 16:55:21  smb
 * Minor changes to sdsuParamRead, sdsuParamWrite and SDSU_NUM_HEADER_BYTES
 *
 * Revision 1.24  1998/07/27 08:34:10  smb
 * sdsuParamDnLoad added.
 *
 * Revision 1.23  1998/07/23 10:09:11  smb
 ...
 *
 * Revision 1.22  1998/07/20 11:34:34  smb
 * strcpy of non-constant strings replaced by strncpy
 *
 * Revision 1.21  1998/07/17 12:07:08  anj
 * Set SDSU card addresses; modified sdsuContextCreate to check for
 * absent cards without crashing when sysextLib not used.
 *
 * Revision 1.20  1998/07/16 15:55:04  anj
 * Added sdsuParamWRP and reverted sdsuParamWrite to use WRM.
 *
 * Revision 1.19  1998/07/15 17:44:02  anj
 * Fixed sdsuReset so it expects the SYR reply from TIM.
 *
 * Revision 1.18  1998/07/15 17:00:22  anj
 ...
 *
 * Revision 1.17  1998/07/14 15:52:01  anj
 * With SMB, working!
 *
 * Revision 1.16  1998/07/13 14:04:25  smb
 * Fixed bug in areStringsSame.
 *
 * Revision 1.15  1998/07/09 15:17:46  smb
 * sysextProcNumGet replaced with sysProcNumGet
 *
 * Revision 1.14  1998/06/30 14:07:16  smb
 * Ensure everything works when sysextLib and mpPipeDrv removed. Fixed mistakes.
 *
 * Revision 1.13  1998/06/30 09:46:53  smb
 * Fixed typing mistake - ifdef should have been ifndef. Sorry
 *
 * Revision 1.12  1998/06/29 16:47:03  smb
 * Allow dependency on sysextLib to be removed by defining NO_SYSEXTLIB macro.
 *
 * Revision 1.11  1998/06/29 13:22:25  smb
 * Added simulation mode. API to sdsuContextCreate changed.
 *
 * Revision 1.10  1998/06/24 17:49:12  anj
 * Minor changes, including the odd header comment bug.
 *
 * Revision 1.9  1998/06/18 16:04:40  anj
 * Tim Hardy's changes plus DSP symbol lookups for paramters.
 *
 * Revision 1.8  1998/01/14 13:00:50  anj
 ...
 *
 * Revision 1.7  1998/01/06 10:32:27  smb
 * External modules and includes added to documentation
 *
 * Revision 1.6  1998/01/05 17:33:20  smb
 * Several ERROR_MSG_NONEs replaced by actual error messages
 *
 * Revision 1.5  1997/12/16 16:20:15  smb
 * Fixed mistake in RCS keyword
 *
 * Revision 1.4  1997/12/16 16:10:21  smb
 * sdsuVersionGet now gets version from CVS
 *
 * Revision 1.3  1997/12/11 15:35:07  smb
 * Expand terse error messages - no changes to algorithms
 *
 * Revision 1.2  1997/12/10 15:00:47  smb
 * Add Brian Leckie comments
 *
 * Revision 1.1.1.1  1997/11/28 11:46:16  anj
...
/* defines */
	...
	/* Modes of operation. Changed for debugging purposes only */
...
#define	CMD_BUF_NWORD				(2 + MAX_NUM_CMD_ARG)		/* # words in command		*/
											/* buffer is one header word, one command word	*/
											/* and up to MAX_NUM_CMD_ARG argument words		*/
#define	REP_BUF_NWORD				32		/* # words in circular reply buffer: must agree	*/
											/* with what is implemented in the VME DSP code */
#define	PRINT_FUNC_WORDS_PER_LINE	8		/* # words/line for sdsuPrint[Rep|Cmd]Buf()		*/
#define	D_FUNC_WORDS_PER_LINE		4		/* # words/line for sdsuD()						*/

	/* File download/upload parameters */

#define	FILE_LOAD_BUFFER_SIZE		1024	/* Working buffer size (# 32-bit words)	used	*/
											/* during memory up- and down-loading			*/
	/*
	 * Object Module Format (OMF) record definitions. There are six record types
	 * defined. See also the enums OMF_FIELD_IDENT_name which are numeric
	 * identifiers assigned to each record type and used internally by sdsuLib.
	 */

#define	OMF_FIELD_START			"_START"
#define	OMF_FIELD_END			"_END"
#define	OMF_FIELD_DATA			"_DATA"
#define	OMF_FIELD_BLOCKDATA		"_BLOCKDATA"
#define	OMF_FIELD_SYMBOL		"_SYMBOL"
#define	OMF_FIELD_COMMENT		"_COMMENT"
...
#define	TIMEOUT_READ_DATA_SEC		1.0e-2			/* Timeout (s) polling for data per packet	*/
 ...
 * N O T E:
 *
 * Below is an old value of this parameter which someone (not me) has commented out and
 * replaced with the 10.0e-06 values above. I am not sure what is going on here. On an
 * mv167 it is not possible to wait for shorter than 1.6667e-02 seconds because the
 * system clock rate is 60 Hz, and 10.0e-06 seconds is in the realm of fantasy. I think
 * both these sets of values will result in a taskDelay(0), which effectively means no
 * wait at all but allow a task context switch. SMB - 11 December 1998.
 */

#define	DELAY_READ_REPLY_SEC		1.0e-02		/* Sleep time (s) polling for reply	(OLD)	*/

#endif /* COMMENTED OUT BY SOMEONE */


	/* Command/reply symbol-table defs */
...
#define	SYMBOL_TYPE_MASK			((SYM_TYPE) 1)	/* Mask for symbol types				*/
	 ...
	 * get OR'd together in the command-definition table (sdsuCmdTable[]) to generate
	 * a set of valid replies to each command.
	 */
...
enum						/* Numeric identifiers used internally for OMF record types */
	{
...
LOCAL	SYMTAB_ID	sdsuSymTab = NULL;			/* Symbol table for defined cmds & replies	*/
LOCAL	SEM_ID		sdsuAtomicSem = NULL;		/* Ensures atomicity for certain operations */
												/* which would otherwise conflict with the	*/
												/* need for sdsuLib to be re-entrant		*/

LOCAL	SEM_ID		sdsuSyncSem = NULL;			/* Counting semaphore used to control how	*/
												/* many WFSs can operate simultaneously.	*/
	 ...
	 * static they can be "remembered" between successive calls to sdsuD, thus allowing
	 * the arguments to this routine to be ommitted following the first call (analogous
	 * to the VxWorks d() routine - see usrLib).
	 */
...
LOCAL	int			sdsuDnWord = 4 * D_FUNC_WORDS_PER_LINE;
	 ...
	 * static they can be "remembered" between successive calls to sdsuM, thus allowing
	 * the arguments to this routine to be ommitted following the first call (analogous
	 * to the VxWorks m() routine - see usrLib).
	 */
...
LOCAL	uint32		sdsuMptr = SDSU_MEM_START_P;
	...
	/*
	 * The following table defines all valid primitive commands recognised by sdsuLib.
	 * New commands can be added as and when required by adding an additional array
	 * element of type SDSU_CMD_DEF = {pString, nArg, nRep, timeout, replyExpected}
	 * where pString is the command mnemonic, nArg the number of command arguments,
	 * nRep the number of arguments expected in the reply, timeout is the time allowed
	 * in microseconds for the command to execute before sdsuLib will generate a
	 * time-out error and replyExpected defines the replies which the controller is
	 * expected to produce in response to each command (obtained by ORing together each
	 * possible reply).
	 *
	 ...
	 * <<<< values.																		  >>>>
	 *
	 * I have increased some of the really small timeouts and have adopted the policy of
	 * increasing a timeout whenever it turns out to be a problem. SMB - 13 November 1998.
	 */

LOCAL SDSU_CMD_DEF sdsuCmdTable [] =
	{
		{"ABT", 0, 0,  2000000,	BIT_FIELD_REPLY_DON},		/* Abort the previous RDC command.	*/
		{"IDL", 0, 0,   100000,	BIT_FIELD_REPLY_DON },		/* IDL command used for debugging.	*/
	...
	/*
	 * The following table defines all valid replies to primitive commands recognised
	 * by sdsuLib. New replies can be added as and when required by adding an additional
	 * array element of type SDSU_REP_DEF = {pString, nArg, replyType} where pString is
	 * the reply mnemonic, nArg the number of reply arguments and replyType identifies
	 * the type of reply.
...
/* LOCAL */	STATUS	sdsu_checkRepBuf (SDSU_ID pContext);
/* LOCAL */	STATUS	sdsu_incRepBufCounter (SDSU_ID pContext, int nIncrement);
...
LOCAL	int		sdsu_makeHeader (SDSU_CMD_DEF * pCmdDef, uint32 sourceId, const uint32 destId,
...
LOCAL	STATUS	sdsu_makeCmd (SDSU_ID pContext, SDSU_CMD_DEF * pCmdDef, const uint32 sourceId,
					const uint32 destId, uint32 * pCmdArg);
LOCAL	int		sdsu_getRecordType (char * pField);
LOCAL	STATUS	sdsu_testTDL (SDSU_ID context, const uint32 destId, const BOOL verbose);
LOCAL	STATUS	sdsu_testRDM (SDSU_ID context, const uint32 destId, uint32 * pMemSpace,
					uint32 * pAddress, const BOOL verbose);
LOCAL	STATUS	sdsu_testWRM (SDSU_ID context, const uint32 destId, uint32 * pMemSpace,
					uint32 * pAddress, const BOOL verbose);
...
LOCAL	void	sdsu_simpleTask (int iContext, int iPacket, int iFrame, int iUseInt);
 ...
 *	(!)	sdsuAtomicSem	(SEM_ID)		Semaphore ensures atomicity of certain operations.
 *
 *	PRIOR REQUIREMENTS:
 *	None
 *
...
STATUS	sdsuLibInit (void)
{
	...
	taskLock ();						/* taskLock() ensures init can only happen once per CPU.	*/
	if (sdsuAtomicSem == NULL)			/* Only initialise if have not previously done so.			*/
	{

		/*
		 * Create a symbol table used to store definitions of SDSU commands (/responses).
		 * Add a symbol for each command found in the arrays sdsuCmdTable[] and sdsuRepTable[].
		 */

		if ((sdsuSymTab = symTblCreate ((int) SYMTAB_HASHSIZE, FALSE, memSysPartId)) == NULL)
		{
			ERROR_SET (0, "Failed to create sdsuSymTab symbol table", ERROR_LOG_SAVE);
			return (ERROR);
		}

		for (index = 0; index < NELEMENTS (sdsuCmdTable); index++)
		{
			if (symAdd (sdsuSymTab, sdsuCmdTable [index].pCmdString, (char *) & sdsuCmdTable [index],
				SYMBOL_TYPE_CMD, SYMBOL_GROUP) == ERROR)
			{
				ERROR_SET1 (0, "Failed to add symbol %s to command table", ERROR_LOG_SAVE,
					sdsuCmdTable [index].pCmdString);
				return (ERROR);
			}
		}
		...
		{
			if (symAdd (sdsuSymTab, sdsuRepTable [index].pRepString, (char *) & sdsuRepTable [index],
				SYMBOL_TYPE_REP, SYMBOL_GROUP) == ERROR)
			{
				ERROR_SET1 (0, "Failed to add symbol %s to response table", ERROR_LOG_SAVE,
					sdsuRepTable [index].pRepString);
				return (ERROR);
			}
			sdsuRepTable [index].repWord = SDSU_STRING_TO_UINT (sdsuRepTable [index].pRepString);
		}

		/*
		 * Create a semaphore which allows certain sdsuLib operations to be atomic. Each
		 * SDSU context structure has another mutex semaphore which ensures that once an
		 * SDSU primitive command has been executed a reply must be read into the same context
		 * structure. However, sdsuAtomicSem is used for global things like connecting to
		 * interrupt sources etc. The existence of this semaphore also acts as a flag which
		 * indicates when the library has been initialised.
		 */

		if ((sdsuAtomicSem = semMCreate (SEM_Q_FIFO | SEM_DELETE_SAFE)) == NULL)
		{
			ERROR_SET (0, "Failed to create sdsuAtomicSem mutex semaphore", ERROR_LOG_SAVE);
			return (ERROR);
		}

		/*
		 * Create a counting semaphore used to arbitrate between different wavefront sensors.
		 * The semaphore is initialised with the number of wavefront that can be used simultaneously.
		 */

		if ((sdsuSyncSem = semCCreate (SEM_Q_PRIORITY,1)) == NULL)
		{
			ERROR_SET (0, "Failed to create sdsuSyncSem counting semaphore", ERROR_LOG_SAVE);
			return (ERROR);
		}
 ...
 *
 *	FUNCTION VALUE:
 *	(STATUS)  OK, or ERROR if the given address could not be mapped to the VME bus.
 *
 *	PURPOSE:
 *	Probes an address to see if an SDSU controller is present
 *
 *	DESCRIPTION:
 *	This function provides a non-intrusive way of testing for the presence of an SDSU VME
 *	card. Unlike sdsuContextCreate, it does not create a context structure or symbol table
 *	and so call be called at will. The function is designed for engineering testing only.
 *
 *	SEE ALSO:
 *	sdsuContextCreate
 *
 ...
 *	commands which involve communication with SDSU controller hardware. It returns
 *	the context ID (a pointer to a structure) which is subsequently used to
 *	maintain information about the status of the controller, including the
 *	circular reply buffer and various configuration & status data. The context
 *	ID ensures that sdsuLib is re-entrant since it holds all data specific
 *	to a given controller: multiple controllers may therefore use sdsuLib
 *	on a single CPU under VxWorks providing that each has a unique context ID.
 *
 ...
 *
 *	POST REQUIREMENTS:
 *	IMPORTANT. After calling this function sdsuSetReplyAddress() needs to be called to
 *	set the reply address before the controller can be used. Alternately, sdsuReset()
 *	may be called to reset the controller and set the reply address.
 *
 *	INCLUDE FILES:
 *	sdsuLib.h
 *	errorLib.h
	...
	)
{
	...
	/* Allocate memory for the context structure, initialising its contents to zero. */

	if ((context = (SDSU_ID) calloc ((size_t) 1, sizeof (SDSU_ID_STRUCT))) == NULL)
	{
	...
	/*
	 * Check whether the controller is to be simulated. In simulation mode no attempt
	 * will be made to use the specified address on the VME bus, and the local address
	 * of the SDSU controller will be set to NULL.
	 */
	...
	if (!simulate)
	{
		...
		if (vxMemProbe((char *) context->pVmeAddress, VX_WRITE, sizeof(uint32), 
						(char *) &i) == ERROR)
		{
			ERROR_SET (S_sdsuLib_INV_CARD_ADDRESS, "SDSU interface not present", ERROR_LOG_SAVE);
			cfree ((char *) context);
			return (NULL);
		}
	 ...
	 * This is just some local memory in which SDSU commands get assembled as a command header,
	 * command mnemonic and any associated command arguments.
	 */

	context->pCmdBuffer = (uint32 *) calloc ((size_t) CMD_BUF_NWORD, sizeof (uint32));
	...
	/*
	 * Allocate memory for the reply buffer (the size of which - CMD_BUF_NWORD - *MUST* agree with
	 * that programmed in the firmware for the SDSU VME DSP). The reply buffer is probably cached
	 * so we have to make sure we invalidate the cache whenever we're about the read it. The reply
	 * buffer must start on a 256-byte boundary.
	 */
	context->pRepBuffer = (uint32 *) memalign (SDSU_REP_BUF_ALIGN, REP_BUF_NWORD * sizeof (uint32));
	...
	/*
	 * Initialise reply buffer by filling it with zeros (since calloc was not used to allocate it),
	...
	if (cacheFlush (DATA_CACHE, context->pRepBuffer, REP_BUF_NWORD * sizeof (uint32)) == ERROR)
	{
	...
	if (((context->paramSyms = symTblCreate ((int) SYMTAB_HASHSIZE, FALSE, memSysPartId)) == NULL))
	{
	...
	/*
	 * Create a semaphore to ensure atomic command-execution/completion.
	 * This semaphore needs SEM_Q_PRIORITY and SEM_INVERSION_SAFE options to ensure
	 * that the higher priority SDSU readout task (spawned by sdsuReadoutOpen) has
	 * priority access to the SDSU controller when it wants it.
	 */

	context->commandSem = semMCreate (SEM_Q_PRIORITY | SEM_DELETE_SAFE | SEM_INVERSION_SAFE);
 ...
 *
 *	DEFICIENCIES:
 *	It is not possible to tidy up the SDSU interrupt vectors, since VxWorks does
 *	not have an intDisconnect function. Repeated cycles of sdsuContextCreate and
 *	sdsuContextDelete will eventually run out of interrupt vectors.
 *-
 */

STATUS	sdsuContextDelete
	(
	SDSU_ID	context
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_INV_STRUCTURE, "Invalid SDSU context", ERROR_LOG_SAVE);
		return (ERROR);
	}

	/*
	 * Abort any current observation, disable SDSU interrupts and delete the SDSU readout task
	 * if one has been spawned.
	 * If any errors occur while doing this they are reported to the user, but they will not
	 * prevent the context being deleted.
	 */
	...
	{
		ERROR_LOG ("Failed to disable SDSU interrupts, but deleting context anyway");
	}

	if ( sdsuReadoutClose (context) == ERROR )
	{
		ERROR_LOG ("Failed to delete readout task, but deleting context anyway");
	}
	...
	/* Free memory previously allocated in sdsuContextCreate() and delete the semaphores */
	...
	)
{
	...
	if (!context->simulate)
	{
		...
		if (what & SDSU_RESET_VME)
		{
			...
			{
				ERROR_SET (0, "Failed to set reply address for VME DSP", ERROR_LOG_SAVE);
				return (ERROR);
			}
		}

		/* Reset remote DSPs by sending an RRS command to VME, but reply comes from TIM. */
...
#endif
			if (symFindByNameAndType (sdsuSymTab, "RRS", (char **) & pCmdDef, & symType,
				SYMBOL_TYPE_CMD, SYMBOL_TYPE_MASK) == ERROR)
			{
				ERROR_SET (S_sdsuLib_INV_COMMAND, "Could not find RRS command definition symbol",
				           ERROR_LOG_SAVE);
				return (ERROR);
			}

			if ((sdsuPrimitiveWrite (context, pCmdDef, SDSU_IDENT_HST, SDSU_IDENT_VME, NULL)
			     == ERROR) ||
				(sdsuPrimitiveRead (context, pCmdDef, SDSU_IDENT_TIM, SDSU_IDENT_HST, NULL)
			     == ERROR)
			   )
			{
 ...
 *
 *	The contents of pTestMask on return from this routine reflect the result of the
 *	tests performed - in the event of successful tests then *pTestMask is set to zero,
 *	while if either test fails then the corresponding bit in *pTestMask (as defined
 *	in the above table) will be set. Note that the tests are aborted immediately on
 *	detection of an error so that only one bit in *pTestMask should ever be set on
 *	return. Also, in the event that more than one DSP is tested (more than one bit
 *	in dspIdentMask set on entry to this routine), then if an error is detected in
 *	one of the tests there is no way of determining which DSP failed the given test.
	...
	)
{
	...
	* pTestMask = 0;										/* Assume all tests will be passed	*/
	...
	/*
	 * For each DSP, determine whether the requested tests are to be performed, then execute each
	 * test in turn. Return at the first test to fail or when all have been completed. If a test
	 * does fail, set the returned test mask, *pTestMask, to identify the failed test.
	 */
	...
	for (ident = SDSU_IDENT_VME; ident < SDSU_IDENT_INVALID; ident++)
	{
		if ((dspIdentMask & (1 << (ident - 1))) != 0)
		{
			...
			{
				if (sdsu_testRDM (context, ident, & memSpace, & address, verbose) == ERROR)
				{
			...
			{
				if (sdsu_testWRM (context, ident, & memSpace, & address, verbose) == ERROR)
				{
	...
	)
{
	...
	for (i=0; ((pFrame != NULL) && (i <= context->nFrames)); i++)	/* Do not allow infinite loop */
	{
 ...
 *	nibble of the 24-bit address (see the SDSU hardware manual), then pMemSpace is
 *	ignored. If the upper nibble of ptr is zero, then the string at pMemSpace
 *	identifies the required memory space and should be one of "x", "y", "p" or "e"
 *	corresponding to X, Y, program and EEPROM memory spaces respectively. nWord gives
 *	the number of 24-bit DSP words to display starting from address ptr. destId
 *	identifies the target DSP to access - this can be a valid DSP ID (see table below)
 *	or may point to a string containing one of "v", "t" or "u" corresponding to the
 *	VME, Timing or Utility DSP respectively.
 *
 ...
 *
 *	The routine stores the last set of parameters which were successfully employed
 *	in a set of static variables and substitutes these for any entry parameters 
 *	which are either zero or NULL. This allows the second and subsequent calls to
 *	the routine to advance through successive "pages" of DSP memory, since the address
 *	(ptr) is incremented by nWord locatations before being saved. Similarly, in many
 *	cases parameters such as the context ID need only be specified the first time
 *	the routine is invoked. Note, however, that in order to allow addresses starting
 *	from ptr=0 to be displayed the value of ptr is only substituted with that used
 *	on the last call to the routine if both ptr=0 and pMemSpace=0.
 *
 *	NOTE:
 *	This function is designed to be invoked from the VxWorks shell
 *
 ...
 *
 *	DEFICIENCIES:
 *	The output produce by this function can be a little confusing if nWord is less than 4.
 *	The address of each word displayed is a combination of the address shown at the left
 *	and the column number the word appears in. If one word is displayed the address shown
 *	on the left may not correspond to the address of the word displayed.
 *-
 */
	...
	)
{
	...
	if ((ptr != NULL) && (sdsu_areStringsSame ((char *) ptr, "-help")))
	{
		...
		printf ("       space = [\"x\"|\"y\"|\"p\"|\"e\"] memory-space to display\n");
		printf ("       nWord = number of words (24-bit word length) to display\n");
	...
	/*
	 * Get any command-line arguments that were not given (or for which the value given was 0) from
	 * the static variables sdsuD__
	 */
	...
	{
		if		(sdsu_areStringsSame ((char *) destId, "v"))	destId = SDSU_IDENT_VME;
		else if	(sdsu_areStringsSame ((char *) destId, "t"))	destId = SDSU_IDENT_TIM;
		else if	(sdsu_areStringsSame ((char *) destId, "u"))	destId = SDSU_IDENT_UTL;
	}
	...
	{
		ERROR_SET (0, "Memory allocation for temporary buffer failed", ERROR_LOG_SAVE);
		return (ERROR);
	}

	/*
	 * If pMemSpace identifies a valid DSP memory space ("x","Y","p" or "e" - case-insensitive)
	 * then OR the mem-space identifier with the starting address. Otherwise, simply ignore the
	 * mem-space identifier string. Also ignore the string if ptr already contains a DSP ident.
	 */
	...
	{
		ERROR_SET1 (0, "Failed to read block of DSP memory at address %#x", ERROR_LOG_SAVE, ptr);
		cfree ((char *) pData);
		return (ERROR);
	}
	...
	for (addr = addr1, i = 0, j = 0; addr <= addr2; addr++)
	{
		if ((addr % D_FUNC_WORDS_PER_LINE) == 0)
		{
			...
			{
				ERROR_SET (S_sdsuLib_INV_PROC_ID, "Invalid DSP target", ERROR_LOG_SAVE);
				cfree ((char *) pData);
				return (ERROR);
			}

			if		((addr & SDSU_MEM_SPACE_MASK) == SDSU_MEM_SPACE_X)	printf ("X:");
			else if	((addr & SDSU_MEM_SPACE_MASK) == SDSU_MEM_SPACE_Y)	printf ("Y:");
			else if	((addr & SDSU_MEM_SPACE_MASK) == SDSU_MEM_SPACE_P)	printf ("P:");
			else 														printf ("?:");
		...
		if (((addr + 1) % D_FUNC_WORDS_PER_LINE) == 0) printf ("   *%s*\n", pString);
	}
	...
	/*
	 * Save the parameters adopted on this invocation of the routine in the static variables
	 * sdsuD__, for optional use on the next invocation
	 */
 ...
 *	(>)	pMemSpace	(char *)	String identifies memory space ("x","y","p" or "e")
 *	(>)	destId		(uint32)	DSP ID number or pointer to string ID ("v", "t" or "u")
 *	(>)	context		(SDSU_ID)	Context ID
 *
 *	FUNCTION VALUE:
 ...
 *	ignored. If the upper nibble of ptr is zero, then the string at pMemSpace
 *	identifies the required memory space and should be one of "x", "y", "p" or "e"
 *	corresponding to X, Y, program and EEPROM memory spaces respectively. destId
 *	identifies the target DSP to access - this can be a valid DSP ID (see table below)
 *	or may point to a string containing one of "v", "t" or "u" corresponding to the
 *	VME, Timing or Utility DSP respectively.
 *
 ...
 *	in a set of static variables and substitutes these for any entry parameters 
 *	which are either zero or NULL. This allows the second and subsequent calls to
 *	the routine to advance through DSP memory. Similarly, in many
 *	cases parameters such as the context ID need only be specified the first time
 *	the routine is invoked. Note, however, that in order to allow addresses starting
 *	from ptr=0 to be displayed the value of ptr is only substituted with that used
 *	on the last call to the routine if ptr=0 and pMemSpace=0.
 *
 *	NOTE:
 *	This function is designed to be invoked from the VxWorks shell
 *
	...
	)
{
	...
	if ((ptr != NULL) && (sdsu_areStringsSame ((char *) ptr, "-help")))
	{
		...
		printf ("where: addr  = DSP address (with or without mem-space identifier in top nibble)\n");
		printf ("       space = [\"x\"|\"y\"|\"p\"|\"e\"] memory-space to modify\n");
		printf ("       dsp   = [\"v\"|\"t\"|\"u\"|%d|%d|%d] VME, Timing or Utility DSP\n",
			SDSU_IDENT_VME, SDSU_IDENT_TIM, SDSU_IDENT_UTL);
		printf ("       ctxt  = pointer to valid SDSU context structure\n");
		return (OK);
	...
	while (! done)							/* Keep going until a non-numeric character is input	*/
	{
		...
		{
			ERROR_SET1 (0, "Failed to read DSP memory at address %#x", ERROR_LOG_SAVE, ptr);
			return (ERROR);
		}
		...
		{
			i = 1;
			while ((pString [i++] = getchar ()) != 0x0a)			/* ...get all input characters	*/
				;					/* Null loop body */

			data = strtoul (pString, NULL, 16) & 0xffffff;			/* then convert to a digit		*/
			...
			{
				ptr--;
				ERROR_SET1 (0, "Failed to write DSP memory at address %#x", ERROR_LOG_SAVE, ptr);
				return (ERROR);
			}
		}
	}
	...
	/*
	 * Save the parameters adopted on this invocation of the routine in the static variables
	 * sdsuM__, for optional use on the next invocation
	 */
 ...
 *
 *	DESCRIPTION:
 *	This routine prints the SDSU command buffer at the VxWorks console. The buffer
 *	should hold the last sequence of commands issued to the controller.
 *	Those buffer locations which contain recognisable SDSU command words are
 *	displayed as ASCII strings, all other words are in hexadecimal format.
 *	If printAll is TRUE the entire buffer is printed (typically 32 words), otherwise
 *	only the last command is displayed.
 *
 *	EXAMPLES:
	...
	)
{
	...
	for (n = 1; n <= CMD_BUF_NWORD; n++)
	{
		...
		if (symFindByNameAndType (sdsuSymTab, pString, (char **) & pCmdDef, & symType,
			SYMBOL_TYPE_CMD, SYMBOL_TYPE_MASK) == OK)
		{
			printf ("%6s", pString);
		}
		else if (((context->pCmdBuffer [n - 1] & 0xffffff) == SDSU_CMD_WORD_EMPTY) && (! printAll))
		{
			printf ("\n");
			break;
		}
 ...
 *
 *	DESCRIPTION:
 *	This routine prints the SDSU circular reply buffer at the VxWorks console. The buffer
 *	is cleared by sdsuLib when it wraps over and the current buffer pointer, i.e. the
 *	location to which the next reply word is expected to be written bracketed by
 *	left- and right-angle brackets ('>' and '<'). Those buffer locations which contain
 *	recognisable SDSU reply words are displayed as ASCII strings, all other words are in
 *	hexadecimal format.
 *
 *	The most significant byte of each reply word (which is not used by the SDSU
 *	hardware) is set to 0xff whenever the word contains data written by the SDSU DSP
 *	which has not yet been read by the VxWorks software. These words are bracketed
 *	by '!' characters. If the word at the reply buffer pointer is flagged in this way
 *	it is bracketed by '*' instead of '>' and '<'.
 *
 *	EXAMPLE:
	...
	)
{
	...
	if (cacheInvalidate (DATA_CACHE, context->pRepBuffer, REP_BUF_NWORD * sizeof (uint32)) == ERROR)
	{
	...
	{
		if ((n-1 == context->repBufCounter) && ((context->pRepBuffer) [n - 1] & 0xff000000))
			printf ("*");
		...
		reply = (context->pRepBuffer) [n - 1] & 0xffffff;		/* Get reply word from the buffer	*/
		SDSU_UINT_TO_STRING (reply, pName);						/* ...convert it to a string		*/
 ...
 *
 *	DESCRIPTION:
 *	This routine issues a primitive command to a specified target processor. The
 *	source will normally be the host CPU and the destination will normally be one of
 *	the DSPs - VME, Timing or Utility, as specified in the table.
 *
 *		SDSU_IDENT_HST	=>	Refers to the host CPU
 *		SDSU_IDENT_VME	=>	Refers to VME DSP
 *		SDSU_IDENT_TIM	=>	Refers to Timing DSP
 *		SDSU_IDENT_UTL	=>	Refers to Utility DSP
 *
 *	The command is determined by the structure pCmdDef (the definition of which can
 *	be found in sdsuLib.h). pCmdArg points to an array of command arguments and these
 *	must occur in the order they are written to the DSP. A NULL pointer may be passed
 *	if there are no arguments. This routine is not normally used by application
 *	programs and is included in the public interface as an aid to system diagnostics.
 *
 *	ATOMICITY OF PRIMITIVE COMMANDS:
 *	This routine calls semTake() on a mutex semaphore contained within the SDSU context
 *	structure; the related routine sdsuPrimitiveRead() calls semGive() on the same
 *	samaphore. This semaphore ensures that a primitive command must be completed
 *	before a new one can be issued to a given controller.
 *
 *	EXTERNAL VARIABLES:
 *	None
 *
 ...
 *
 *	DEFICIENCIES:
 *	It is possible for this function to wait forever to take the SDSU command semaphore,
 *	although this is very unlikely to happen.
 *-
 */

STATUS	sdsuPrimitiveWrite
	(
	SDSU_ID			context,
	SDSU_CMD_DEF *	pCmdDef,
	...
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_INV_STRUCTURE, "Reply buffer error - write to controller aborted",
			ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	/* Only write the command to the SDSU hardware if we are not running in simulation mode. */
 ...
 *
 *	DESCRIPTION:
 *	This routine reads the reply to a previously-executed primitive command. The
 *	source will normally be one of the three DSPs - VME, Timing or Utility, and the
 *	destination will normally be the host CPU, as specified in the table.
 *
 *		SDSU_IDENT_HST	=>	Refers to the host CPU
 *		SDSU_IDENT_VME	=>	Refers to VME DSP
 *		SDSU_IDENT_TIM	=>	Refers to Timing DSP
 ...
 *	be found in sdsuLib.h). pRepArg points to an array to which any arguments associated
 *	with the reply will be copied. A NULL pointer may be passed
 *	if there are no arguments expected. This routine is not normally used by application
 *	programs and is included in the public interface as an aid to system diagnostics.
 *
 *	ATOMICITY OF PRIMITIVE COMMANDS:
 ...
 *
 *	DEFICIENCIES:
 *	If the reply contains data which happens to contain the string "ERR" or "SYR" for some 
 *	innocent reason, this function will assume, incorrectly, that an error has occurred.
 *	
 *	This code is horrible, and is unable to cope with an unexpected SYR reply from
 *	the timing DSP (as generated by an RRS command, or by someone pressing the reset
 *	button on the controller).  It needs a rewrite it to
 *	cope with this, but I don't think it's essential right now.  ANJ - 15 July 98.
 *
 *	I agree with ANJ's comment. I have seen the timing DSP reply with SYR just after it
 *	has been switched on, which can make the first attempt to download DSP code to it fail.
 *	The VME board can sometimes respond with a TIM reply if a timeout occurs during an observation,
 *	and this is not handled properly either.
 *	SMB - 17 September 98.
 *-
 */

STATUS	sdsuPrimitiveRead
	(
	SDSU_ID			context,
	SDSU_CMD_DEF *	pCmdDef,
	uint32			sourceId,
	...
	)
{
	...
	if ( headerExpected == 0 )
	{
		ERROR_SET (0, "Failed to construct the expected header word", ERROR_LOG_SAVE);
		semGive (context->commandSem);
		sigsetmask (context->sigMask);
		return (ERROR);
	}
	...
	/*
	 * A reply will only be received if we are dealing with real SDSU hardware, so only
	 * check for a reply if we are not running in simulation mode.
	 */
	...
	if (!context->simulate)
	{
		...
		/*
		 * Enter a loop polling for the reply-header word. Only break from this loop when
		 * a valid header word has been read or after the specified timeout period for the
		 * command. But, before entering this loop poll just once for the reply word since
		 * in most cases a header will already be present and there is no need to fuss around
		 * setting up for timeouts etc.
		 */

		if (cacheInvalidate (DATA_CACHE, context->pRepBuffer, REP_BUF_NWORD * sizeof (uint32))
		    == ERROR)
		{
			ERROR_SET (0, "Cache invalidate for reply buffer failed", ERROR_LOG_SAVE);
			semGive (context->commandSem);
		...
		{
			/* The header wasn't valid on the first poll, so enter the timeout loop */
			...
			{
				taskDelay (SEC_TO_NTICK (DELAY_READ_REPLY_SEC));	/* Twiddle thumbs while waiting	*/
				if (cacheInvalidate (DATA_CACHE, context->pRepBuffer, REP_BUF_NWORD * sizeof (uint32))
				    == ERROR)
				{
					ERROR_SET (0, "Cache invalidate for reply buffer failed", ERROR_LOG_SAVE);
		...
		if ((context->pRepBuffer [context->repBufCounter] & 0xff000000)	== 0)
		{
			...
			ERROR_SET3 (S_sdsuLib_REPLY_TIMEOUT,
			            "Timeout reading header word, expected=%#x, RepBuffer=%p+%d", ERROR_LOG_SAVE,
			            headerExpected, context->pRepBuffer, context->repBufCounter);
		...
		if ((header & 0xffff00)	!= (headerExpected & 0xffff00))
		{
			 ...
			 * 
			 * This might be an SYR from TIM or a TIM from VME (if we were expecting a response
			 * from some other DSP just now), but there's no code in here to
			 * check that and deal with the consequences.
			 */
...
#ifdef DEBUG
		if (sdsuFullDebug)
			printf ("sdsuPrimitiveRead: Read expected reply header (%#x) successfully\n", header);
#endif /* DEBUG */

		if ((context->pRepBuffer [context->repBufCounter] & 0xff000000)	== 0)
		{
			/* The reply word wasn't valid on the first poll.  Enter the timeout loop */
			...
			while (((context->pRepBuffer [context->repBufCounter] & 0xff000000) == 0) &&
				! timeoutExpired (TIMEOUT_READ_REPLY_SEC, & context->timeoutStart))
			{
				taskDelay (SEC_TO_NTICK (DELAY_READ_REPLY_SEC));	/* Twiddle thumbs while waiting	*/
				if (cacheInvalidate (DATA_CACHE, context->pRepBuffer, REP_BUF_NWORD * sizeof (uint32))
				    == ERROR)
		...
		{
			/* Nothing was written to the reply buffer before timeout */
			ERROR_SET2 (S_sdsuLib_REPLY_TIMEOUT, "Timeout reading reply word, RepBuffer=%p+%d",
			            ERROR_LOG_SAVE, context->pRepBuffer, context->repBufCounter);
		...
		/*
		 * If the command can return data, then check that an ERR or SYR are not returned.
		 * (NOTE: It is possible for valid data to contain these values for innocent reasons).
		 */
		...
		if ((pCmdDef->repType & BIT_FIELD_REPLY_DAT) != 0)
		{
			...
			{
				ERROR_SET (S_sdsuLib_ERROR_RETURNED, "ERR returned in reply data argument",
				           ERROR_LOG_SAVE);
			...
			{
				ERROR_SET (S_sdsuLib_ERROR_RETURNED, "SYR returned in reply data argument",
				           ERROR_LOG_SAVE);
...
#ifdef	DEBUG
			if (sdsuFullDebug)
				printf ("sdsuPrimitiveRead: Read 24-bit data word (%#x) successfully\n",
		...
		/* Finished reading the SDSU reply-header and reply word: handle any errors */

		if (! validReplyFlag)
		{
			SDSU_UINT_TO_STRING (reply,pReplyString)
			ERROR_SET2 (S_sdsuLib_REPLY_TIMEOUT, "Invalid SDSU reply, returned value=%#x=%s",
		...
		{
			SDSU_UINT_TO_STRING (reply,pReplyString)
			printf ("sdsuPrimitiveRead: Read expected reply word (%#x=%s) successfully\n",
				reply, pReplyString);
		}
#endif /* DEBUG */
		...
		{
			repArg [repArgCounter++] = (context->pRepBuffer) [context->repBufCounter] & 0xffffff;
 ...
 *
 *	FUNCTION VALUE:
 *	(STATUS)  OK, or ERROR if an error was encountered while executing the command.
 *
 *	PURPOSE:
 ...
 *	sdsuPrimitiveWrite() and sdsuPrimitiveRead(), and takes as the command definition
 *	a 3-character (plus null) string which holds the command mnemonic, e.g. "RDM" to
 *	read DSP memory. The source of the command is by definition always the host
 *	processor but the destination is determined by destId which must be a valid
 *	DSP ID taken from the table below. Command arguments are contained in the array
 *	pCmdArg and any returned values are copied to pRepArg. Either of these two pointers
 ...
 *
 *	ATOMICITY OF PRIMITIVE COMMANDS:
 *	This routine uses a mutex semaphore contained within the SDSU context strucuture
 *	to ensure that primitive commands are executed atomically. This is potentially
 *	important where, for example, more than one VxWorks task may issue commands to
 *	the same controller. Such a situation is somwhat dangerous, however a more likely
 *	scenario is one in which a user needs to issue commands from a VxWorks console
 *	(e.g. in order to determine the state of a controller) while a separate VxWorks
 *	task is also issuing commands. Note that there are no exclusion mechanisms for
 *	applications in which primitive commands are issued to a single SDSU controller
 *	from different VxWorks CPUs, since in this case each CPU will have a separate
 *	mutex semaphore within its own context structure. Such an application requires
 *	inter-CPU communications to provide exclusive to each CPU at any instant.
 *
 *	PRIMITIVE COMMANDS:
 *	A primitive command must be recognised by sdsuLib for it to send it. The fact
 *	that a command is recognised does not, however, guarantee that the
 *	command is supported by the downloaded SDSU software or firmware - this must
 *	be determined from examination of the specific system installation.
 *
 *	EXTERNAL VARIABLES:
 *	(>)	sdsuSymTab	(SYMTAB_ID)		Symbol table ID.
 *
	...
	)
{
	...
	if (symFindByNameAndType (sdsuSymTab, pCommand, (char **) & pCmdDef, & symType,
		SYMBOL_TYPE_CMD, SYMBOL_TYPE_MASK) == ERROR)
	{
		ERROR_SET1 (S_sdsuLib_INV_COMMAND, "Could not find command definition symbol, %s",
	...
	if ((sdsuPrimitiveWrite (context, pCmdDef, SDSU_IDENT_HST, destId, pCmdArg) == ERROR) ||
		(sdsuPrimitiveRead (context, pCmdDef, destId, SDSU_IDENT_HST, pRepArg) == ERROR))
	{
		ERROR_SET (0, ERROR_MSG_NONE, ERROR_LOG_SAVE);	/* There is no extra error info. to add */
		return (ERROR);
	}
	return (OK);
}
	...
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_OMF_PARSE_ERROR, "START record not found in OMF file", ERROR_LOG_SAVE);
		return (ERROR);
	}
	else
	{
		 ...
		 *
		 * "_START <Module id> <Version> <Rev #> <Device #> <Asm Version> <Comment>"
		 *
		...
		for (i = 0; i < 6; i++)
		{
			if (fscanf (fd, "%s", pField) == EOF)
			{
				ERROR_SET (0, "Failed to skip over start record in OMF file", ERROR_LOG_SAVE);
				return (ERROR);
			}
		}
	}
	...
	/*
	 * Allocate some memory for a working buffer used during downloading. This size of this
	 * buffer is not critical: it should be large enough to avoid too many file accesses.
	 */

	if ((pBuffer = (uint32 *) calloc ((size_t) FILE_LOAD_BUFFER_SIZE, sizeof (uint32))) == NULL)
	{
		ERROR_SET (0, "Memory allocation for file load working buffer failed", ERROR_LOG_SAVE);
		return (ERROR);
	}

	/*
	 * Get the first field of the record, then enter a loop parsing each field in turn
	 * until an "end" record is read.
	 */
	...
	while ((recordType = sdsu_getRecordType (pField)) != OMF_FIELD_IDENT_END)
	{
		switch (recordType = sdsu_getRecordType (pField))
		{
			case (OMF_FIELD_IDENT_DATA):
				...
				if (fscanf (fd, "%s", pField) == EOF)
				{
					ERROR_SET (0, "Failed to read field from OMF file", ERROR_LOG_SAVE);
					cfree ((char *) pBuffer);
					return (ERROR);
				}
				...
				else
				{
					ERROR_SET1 (S_sdsuLib_INV_MEM_SPACE,
						"Invalid memory space, %s, read from OMF file", ERROR_LOG_SAVE, pField);
					cfree ((char *) pBuffer);
					return (ERROR);
				}
				...
				address = (uint32) sdsu_atoiBase16 (pField);	/* Convert address to numeric		*/
				...
				/*
				 * Enter a loop reading Code/Data values and processing each until another field-type
				 * is encountered. Another field-type indicates the end of this data field.
				 */

				i = 0;
				while (! sdsu_isRecordType (pField))			/* Reached end of this data record?	*/
				{
						...
						address <= finish)
					{
						...
						{
							if (sdsuPrimitiveMultiWRM (context, destId, startAddress, pBuffer, i)
								== ERROR)
							{
								ERROR_SET1 (0, "Failed to write block of DSP memory at address %#x",
				...
				if (i != 0)									/* Download anything left in the		*/
															/* working buffer						*/
				{
					if (sdsuPrimitiveMultiWRM (context, destId, startAddress, pBuffer, i) == ERROR)
					{
						ERROR_SET1 (0, "Failed to write block of DSP memory at address %#x",
							ERROR_LOG_SAVE, startAddress);
						cfree ((char *) pBuffer);
			...
			case (OMF_FIELD_IDENT_SYMBOL):
				...
				if		(sdsu_areStringsSame (pField, "P"))	memSpace = SDSU_MEM_SPACE_P;
				else if	(sdsu_areStringsSame (pField, "X"))	memSpace = SDSU_MEM_SPACE_X;
				else if	(sdsu_areStringsSame (pField, "Y"))	memSpace = SDSU_MEM_SPACE_Y;
				else if	(sdsu_areStringsSame (pField, "N"))	memSpace = SDSU_MEM_SPACE_NONE;
				else
				{
					ERROR_SET1 (S_sdsuLib_INV_MEM_SPACE, "Invalid memory space, %s", ERROR_LOG_SAVE,
						pField);
					cfree ((char *) pBuffer);
					return (ERROR);
				}
				...
				if (fscanf (fd, "%s", pField) == EOF)
				{
					ERROR_SET (S_sdsuLib_INV_OMF_FIELD, "Failed to read field from OMF file",
						ERROR_LOG_SAVE);
					cfree ((char *) pBuffer);
					return (ERROR);
				}
				...
				while (!sdsu_isRecordType (pField))
				{
					...
					if ((memSpace == SDSU_MEM_SPACE_P) &&	/* If it's P but in the E range ...		*/
						(address >= (SDSU_MEM_START_E & ~SDSU_MEM_SPACE_MASK)) &&
						(address <= (SDSU_MEM_END_E & ~SDSU_MEM_SPACE_MASK)))
					{
					...
					if (symAdd(context->paramSyms, symName, (char *) address, (SYM_TYPE) destId, 0))
					{
						ERROR_SET1 (0, "Error adding parameter symbol, %s", ERROR_LOG_SAVE, symName);
						cfree ((char *) pBuffer);
						return (ERROR);
					}
			...
			case (OMF_FIELD_IDENT_BLOCKDATA):
															...
															/* working buffer						*/
				{
					if (sdsuPrimitiveMultiWRM (context, destId, startAddress, pBuffer, i) == ERROR)
					{
						ERROR_SET (0, "Failed to write block of DSP memory", ERROR_LOG_SAVE);
						cfree ((char *) pBuffer);
						return (ERROR);
					}
				}
			...
			case (OMF_FIELD_IDENT_START):
				...
				/* Another "start" record was encountered. We don't expect one at this stage */

				ERROR_SET (S_sdsuLib_OMF_PARSE_ERROR, "Too many START records in OMF file",
					ERROR_LOG_SAVE);
				cfree ((char *) pBuffer);
				return (ERROR);
			...
			case (OMF_FIELD_IDENT_INVALID):
				...
				ERROR_SET (S_sdsuLib_INV_OMF_FIELD, "Invalid record in OMF file", ERROR_LOG_SAVE);
				cfree ((char *) pBuffer);
				return (ERROR);

			default:

				if (fscanf (fd, "%s", pField) == EOF)	/* Read and dump any un-supported records */
				{
					ERROR_SET (0, "Failed to read field from OMF file", ERROR_LOG_SAVE);
					cfree ((char *) pBuffer);
					return (ERROR);
	...
	)
{
	...
	/*
	 * Get the range of address to upload in each memory space. Then process each block of
	 * memory as a separate data record.
	 */
	...
	for (i = 0; i < SDSU_MEM_INDEX_NDEF; i++)
	{
		switch (i)
		{
			/* Write the "type" word and the memory-space for the current data record */
			...
			default:
				ERROR_SET1 (S_sdsuLib_INV_MEM_SPACE, "Invalid memory space, %d", ERROR_LOG_SAVE, i);
				return (ERROR);
		}
		...
		if (pStart [i] <= pFinish [i])
		{
			...
			/* Allocate a working buffer for the data (initialising its contents to zero). */

			if ((pBuffer = (uint32 *) calloc ((size_t) pFinish [i] - pStart [i] + 1,
				sizeof (uint32))) == NULL)
			{
				ERROR_SET (0, "Memory allocation for working buffer failed", ERROR_LOG_SAVE);
				return (ERROR);
			}

			/* Read the data. */	

			if (sdsuPrimitiveMultiRDM (context, destId, pStart [i], pBuffer, pFinish [i]
				- pStart [i] + 1) == ERROR)
			{
			...
			for (j = 0; j < pFinish [i] - pStart [i] + 1; j++)
			{
				...
				{
					ERROR_SET (0, "Failed to write to data record file", ERROR_LOG_SAVE);
					cfree ((char *) pBuffer);
					return (ERROR);
				}
	...
	)
{
	...
	if (strstr (pIoFileName, ".lod") == NULL && strstr (pIoFileName, ".LOD") == NULL)
		strcat (pIoFileName, ".lod");

	if ((pIoFile = fopen (pIoFileName, "r")) == NULL)
	{
		ERROR_SET1 (0, "Failed to open OMF file, %s", ERROR_LOG_SAVE, pIoFileName);
		return (ERROR);
	}

	/*
	 * If the range of addresses to be downloaded to is limited then read the required ranges
	 * into the 1st element of the start[][] and finish[][] arrays. If the range is un-limited,
	 * load the defined start and end addresses for each memory space.
	 */
	...
	/*
	 * Mask off SDSU memory-space identifier bits: the dnload routine doesn't need use them.
	 */
	...
	/*
	 * Initialise the 2nd element of start[][], finish[][] arrays such that the start
	 * address > finish address, thus disabling any downloading on the seconds pass
	 * through the download routine.
	 */
	...
	{
		if (sdsuMemoryDnload (context, pIoFile, destId, & start [i][0], & finish [i][0]) == ERROR)
		{
			ERROR_SET (0, ERROR_MSG_NONE, ERROR_LOG_SAVE);	/* No additional error info. needed.	*/
			fclose (pIoFile);
			return (ERROR);
		}
		rewind (pIoFile);
	...
	)
{
	...
	if (limitAdrsRange)
	{
		...
		printf ("         P:Start P:End: ");
		scanf ("%x %x", & start [SDSU_MEM_INDEX_P],
			& finish [SDSU_MEM_INDEX_P]);

		printf ("         X:Start X:End: ");
		scanf ("%x %x", & start [SDSU_MEM_INDEX_X],
			& finish [SDSU_MEM_INDEX_X]);

		printf ("         Y:Start Y:End: ");
		scanf ("%x %x", & start [SDSU_MEM_INDEX_Y],
			& finish [SDSU_MEM_INDEX_Y]);
	}
	else
	{
	...
	/*
	 * Mask off SDSU memory-space identifier bits: the upload routine doesn't need use them.
	 */
	...
	if (sdsuMemoryUpload (context, pIoFile, destId, & start [0], & finish [0]) == ERROR)
	{
		fclose (pIoFile);
		ERROR_SET (0, ERROR_MSG_NONE, ERROR_LOG_SAVE);		/* No additional error info. needed.	*/
		return (ERROR);
 ...
 *
 *	FUNCTION VALUE:
 *	(STATUS)	OK, or ERROR if an error occurred or the buffer could not be allocated.
 *
 *	PURPOSE:
 ...
 *
 *	REFERENCES:
 *	See Gemini Interface Control Document ICD 1.6/1.10 for a description of the SDSU
 *	data packet format.
 *
 *	INCLUDE FILES:
 *	sdsuLib.h
	...
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_INV_STRUCTURE, "Data buffer already allocated", ERROR_LOG_SAVE);
		return (ERROR);
	}

	/*
	 * Calculate the size of the data buffer needed to hold the specified number
	 * of frames. The data header size is calculated by subtracting the pointer to
	 * the beginning of the SDSU_FRAME structure from the pointer to the beginning
	 * of the data, assuming that sizeof(char) is one byte.
	 */

	frameSize =	((char *) & pFrame->pixel[0]) - ((char *) pFrame) +		/* Header size		*/
	...
	/*
	 * Create a message queue for communication within the readout task, giving the message queue a
	 * buffer of sufficient size to allow all the available frames to be queued.
	 */
	
	context->frameQueue = msgQCreate (nFrames, sizeof (SDSU_FRAME *), MSG_Q_FIFO);
 ...
 *
 *	DEFICIENCIES:
 *	All frames reserved with sdsuFrameReserve will be lost when this function is
 *	executed.
 *-
 */

STATUS	sdsuBufferDelete
	(
	SDSU_ID			context
	)
{
	...
	if (context->pDataBuffer == NULL)
	{
		ERROR_SET (S_sdsuLib_INV_STRUCTURE, "Data buffer not allocated", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	for (frame = 0; frame < context->nFrames; frame++, pBuffer += context->frameSize)
	{
		pFrame = (SDSU_FRAME *) (int) pBuffer;
		semDelete (pFrame->frameSem);
	}
...
#ifdef DEBUG
	printf ("sdsuBufferDelete: Freeing data buffer at %p\n", context->pDataBuffer);
#endif /* DEBUG */
 ...
 *	frame becomes available, or until the timeout elapses, if it does not find a
 *	frame free immediately. The function will not wait if NO_WAIT is specified
 *	and will wait forever if WAIT_FOREVER is specified.
 *
 ...
 *	(started by calling sdsuReadoutOpen) do not need to use sdsuFrameFind at all,
 *	as it will be called automatically by the readout task before each frame callback.
 *
 *	EXAMPLE:
 *v
 ...
 *v		sdsuFrameRelease			- Only now is the frame returned to the free list (readout task)
 *v
 *
 *	SEE ALSO:
 *	sdsuBufferCreate
	...
	)
{
	...
	/*
	 * Attempt to take, with the specified timeout. the counting semaphore controlling the free list.
	 * This semaphore can be taken as long as there is at least one frame on the free list.
	 */

	if ( semTake (context->bufferSem, timeout) == ERROR )
	{
		ERROR_SET (S_sdsuLib_SYNC_ERROR, "No unreserved frames available", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	/*
	 * Take the frame from the free list and change the free list pointer to point to
	 * the next frame in the list.
	 *
	 * This next bit must be uninterruptible, so interrupts and task scheduling are disabled.
	 */
	...
	/*
	 * Initialise the frame header and set the pNext pointer to NULL to show this frame
	 * is no longer part of the free list.
	 */
 ...
 *
 *	DESCRIPTION:
 *	This routine reserves the frame which has already been selected from the allocated
 *	data buffer by a previous call to sdsuFrameFind. The purpose of this is to
 *	flag the data as valuable to prevent the frame from being re-used until
 *	other operations on it are completed. The function can be called as many times
 *	as required by different tasks. The effect of the function is reversed by calls
 *	to sdsuFrameRelease.
 *
 *	The function is designed to be used within application code executed as a callback
 *	by sdsu_readTask. sdsu_readTask will automatically return a frame to the free list unless
 *	it is specifically reserved by the application code.
 *
 *	EXAMPLE:
 *v
	...
	)
{
	...
	if (pFrame == NULL)
	{
		ERROR_SET (S_sdsuLib_INV_PARAM_VAL, "Invalid frame pointer", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	{
		ERROR_SET (S_sdsuLib_INV_PARAM_VAL, "Invalid frame semaphore", ERROR_LOG_SAVE);
		return (ERROR);
	}
 ...
 *
 *	FUNCTION VALUE:
 *	(STATUS)	OK, or ERROR if an error occurred or the frame could not be released.
 *
 *	PURPOSE:
 ...
 *
 *	This function assumes that any error from semTake() means that the counting semaphore is
 *	empty and the frame can be returned to the free list. On rare occasions this error might
 *	be due to an invalid semaphore ID. Ideally, this function should check the errno variable
 *	to distinguish the kind of error that has happened. SMB - 14 December 1998.
 *-
 */

STATUS	sdsuFrameRelease
	(
	SDSU_ID			context,
	SDSU_FRAME *	pFrame
	)
{
	 ...
	 * The call below will return ERROR when the count has reached zero, indicating
	 * that the frame is not reserved and can be returned to the free list.
	 */
	...
	if (status == ERROR)
	{
		...
		/*
		 * First check this frame is not already present on the free list, which might happen
		 * if sdsuFrameRelease is called too many times.
		 */

		found = FALSE;
		pTestFrame = context->pFreeList;
		for (i=0; ( (!found) && (pTestFrame != NULL) && (i < context->nFrames)); i++)
		{
		...
		{
			ERROR_SET (S_sdsuLib_INV_STRUCTURE, "Frame is already on the free list", ERROR_LOG_SAVE);
			return (ERROR);
		}
	}
 ...
 *
 *	DESCRIPTION:
 *	This routine displays the contents of the frame structure pointed to by pFrame.
 *
 *	NOTE:
 *	This function is useful when invoked from the VxWorks shell
 *
	...
	)
{
	...
	/* Check whether the SDSU context and frame buffer address provided are valid. */
	...
	if (pFrame == NULL)
	{
		ERROR_SET (S_sdsuLib_INV_PARAM_VAL, "Frame pointer not provided", ERROR_LOG_SAVE);
		return (ERROR);
	}
	 ...
	 *
	 * Note that the address given is the start of the data header in the SDSU_FRAME structure.
	 * When the SDSU controller writes its data it will automatically overwrite the pFrame->header
	 * structure as well as the memory pointed to by pFrame->pixel.
	 */
	...
	/*
	 * Load the high and low parts of the frame buffer address into the VME DSP, setting the
	 * SDSU_NEW_FBA_FLAG bit to make the DSP recognise that a new address has been loaded.
	 */

	if ( (sdsuParamWrite (context, SDSU_IDENT_VME, "V_FBALO", dmaAddress & 0xffff) == ERROR) ||
		 (sdsuParamWrite (context, SDSU_IDENT_VME, "V_FBAHI",
		                 ((dmaAddress >> 16) | SDSU_NEW_FBA_FLAG)) == ERROR)
	   )
	{
		ERROR_SET (0, "Frame buffer address could not be set in VME DSP", ERROR_LOG_SAVE);
		return ERROR;
	}
 ...
 *
 *	DEFICIENCIES:
 *	There is no way of confirming that the interrupt level SDSU_VME_INT_LEVEL is correct
 *	for this particular SDSU VME card, because it's set by jumper.
 *	
 ...
 *
 *	I have modified this function so that interrupt vectors are not allocated for
 *	interrupts that are not enabled. This should delay the running out of vectors.
 *	SMB - 3 December 1998.
 *
 *	It is possible for this function to wait forever for the sdsuAtomicSem semaphore.
 *	A timeout should be used if this ever becomes a problem.
 *-
 */

STATUS	sdsuIntConnect
	(
	SDSU_ID 	context,
	uint32		intEnable
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_NO_INT_AVAILABLE, "Run out of interrupt vectors", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	if ( ((intEnable & SDSU_PACKET_INT_ENABLE) != 0) && (context->packetIntNum == 0) )
	{
		...
		if ( intConnect (INUM_TO_IVEC (context->packetIntNum), sdsu_packetSyncIsr, (int) context)
		      == ERROR
		   )
		{
			ERROR_SET (0, "Failed to connect packet interrupt service routine", ERROR_LOG_SAVE);
	...
	if ( ((intEnable & SDSU_FRAME_INT_ENABLE) != 0) && (context->frameIntNum == 0) )
	{
...
#ifdef DEBUG
		printf ("sdsuIntConnect: Connecting frame interrupts to vector %d (%p).\n",
		...
		if ( intConnect (INUM_TO_IVEC (context->frameIntNum), sdsu_frameSyncIsr, (int) context)
		      == ERROR
		   )
		{
			ERROR_SET (0, "Failed to connect frame interrupt service routine", ERROR_LOG_SAVE);
	...
	{
		if (sdsuParamWrite (context, SDSU_IDENT_VME, "V_PIID", context->packetIntNum << 16) == ERROR)
		{
			ERROR_SET (0, "Failed to set SDSU packet interrupt vector parameter", ERROR_LOG_SAVE);
			return (ERROR);
		}
	}
	else
	...
	if ( ((intEnable & SDSU_FRAME_INT_ENABLE) != 0) &&
	     (sdsuParamWrite (context, SDSU_IDENT_VME, "V_FIID", context->frameIntNum << 16) == ERROR)
	   )
	{
		ERROR_SET (0, "Failed to set SDSU frame interrupt vector parameter", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	if ( sdsuParamWrite (context, SDSU_IDENT_VME, "V_INT_EN", intEnable) == ERROR )
	{
	...
	)
{
	...
	if ( ((intEnable & SDSU_FRAME_INT_ENABLE) != 0) && (context->frameIntNum == 0) )
	{
...
#ifdef DEBUG
		printf ("sdsuSimpleIntConnect: Connecting frame interrupts to vector %d (%p).\n",
			context->frameIntNum, INUM_TO_IVEC (context->frameIntNum));
#endif	
		...
		if ( intConnect (INUM_TO_IVEC (context->frameIntNum), sdsu_simpleSyncIsr, (int) context)
		      == ERROR
		   )
		{
			ERROR_SET (0, "Failed to connect frame interrupt service routine", ERROR_LOG_SAVE);
	...
	if ( sdsuParamWrite (context, SDSU_IDENT_VME, "V_FIID", context->frameIntNum << 16) == ERROR)
	{
		ERROR_SET (0, "Failed to set SDSU frame interrupt vector parameter", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	if ( sdsuParamWrite (context, SDSU_IDENT_VME, "V_PIID", 0) == ERROR)
	{
		ERROR_SET (0, "Failed to zero SDSU packet interrupt vector parameter", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	if ( sdsuParamWrite (context, SDSU_IDENT_VME, "V_INT_EN", SDSU_FRAME_INT_ENABLE) == ERROR )
	{
		ERROR_SET (0, "Failed to enable SDSU interrupts", ERROR_LOG_SAVE);
		return (ERROR);
	}
 ...
 *
 *	DESCRIPTION:
 *	This is the interrupt service routine used for packet-synchronisation during
	...
	)
{
	...
	if ( context != NULL )
	{
		 ...
		 * the signal handler defined by sdsu_readTask (sdsu_readSignal) to be executed.
		 */
		...
		/* Trigger the processing of the last packet by giving the packet sync semaphore. */
 ...
 *
 *	PURPOSE:
 *	Interrupt service routine for frame-sync interrupt from SDSU controller - simple version
 *
 *	DESCRIPTION:
 *	This is the simple version of the interrupt service routine used for frame-synchronisation during
 *	readout of the SDSU controller.
 *
 *	PRIOR REQUIREMENTS:
 *	None
	...
	)
{
	...
	if ( context != NULL )
	{
		...
		/* Trigger the processing of the frame by giving the packet sync semaphore with interrupts disabled. */
 ...
 *
 *	DESCRIPTION:
 *	This function simulates a packet sync interrupt from the SDSU controller, and
 *	may be used to trigger a packet sync when the controller is being simulated.
 *
 *	PRIOR REQUIREMENTS:
 *	None
 *
	...
	)
{
	...
	/*
	 * Check the SDSU context is valid and give a warning if this function is called
	 * with packet sync interrupts enabled when not in simulation mode.
	 */
	...
	if ( (!context->simulate) && (context->packetIntNum != 0) )
	{
		MESSAGE_LOG (MSG_WARNING,
			"WARNING: Packet interrupts enabled and SDSU controller not being simulated");
	}
 ...
 *
 *	DESCRIPTION:
 *	This function simulates a frame sync interrupt from the SDSU controller, and
 *	may be used to trigger a frame sync when the controller is being simulated.
 *
 *	PRIOR REQUIREMENTS:
 *	None
 *
	...
	)
{
	/*
	 * Check the SDSU context is valid and give a warning if this function is called
	 * with frame sync interrupts enabled when not in simulation mode.
	 */
 ...
 *
 *	DESCRIPTION:
 *	This function simulates a simple frame sync interrupt from the SDSU controller, and
 *	may be used to trigger a frame sync when the controller is being simulated.
 *
 *	PRIOR REQUIREMENTS:
 *	None
	...
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_READOUT_ACTIVE, "Readout task already exists", ERROR_LOG_SAVE);
		return (ERROR);
	}
	
	/*
	 * What name shall I use? Make one up based on who we're called from.
	 * An extra 9 bytes is allocated to allow the ":readout" string to be appended.
	 */
	
	myName = taskName(0);
	if ((readName = (char *) malloc(strlen(myName) + 9)) == NULL)
	{
		ERROR_SET(0, "Memory allocation for readout task name failed", ERROR_LOG_SAVE);
		return (ERROR);
	}
	...
	{
		ERROR_SET1 (0, "Failed to spawn readout task, %s", ERROR_LOG_SAVE, readName);
		context->readTask = 0;
		free (readName);
		return (ERROR);
	}
	...
	if ( useInterrupts )
	{
		...
		{
			ERROR_SET (0, "Readout open failed - failed to connect interrupts", ERROR_LOG_SAVE);
			return (ERROR);
		}
	}
 ...
 *
 *	DESCRIPTION:
 *	This routine will start a SIMPLE VERSION OF THE readout task for the indicated controller,
 *	setting the application callback routines to the packetCall and frameCall
 *	functions. These may be passed as NULL if not required by the application.
 *	The useInterrupts parameter should normally be TRUE but can be set FALSE
 *	to operate the SDSU controller without interrupts (in which case the
	...
	)
{
	...
	context->readTask = taskSpawn (readName, SDSU_APPLICATION_PRIORITY+nice, 
		VX_FP_TASK, SDSU_READTASK_STACKSIZE,
		(FUNCPTR) sdsu_simpleTask, (int) context, (int) packetCall, (int) frameCall,
		(int) useInterrupts, 0, 0, 0, 0, 0, 0);
	
	if (context->readTask == ERROR)
	{
		ERROR_SET1 (0, "Failed to spawn simple readout task, %s", ERROR_LOG_SAVE, readName);
		context->readTask = 0;
		free (readName);
		return (ERROR);
	}
	...
	/*
	 * If requested, connect interrupts to the card. The packet callback is ignored.
	 */
	...
	if ( useInterrupts )
	{
		...
		{
			ERROR_SET (0, "Readout open failed - failed to connect simple interrupts",
				ERROR_LOG_SAVE);
			return (ERROR);
		}
	}
 ...
 *	allowed to spend as much elapsed time as it likes processing the packet data, 
 *	although on average the processing time must not exceed the time between
 *	readouts or the system will run out of image buffers.  When the frame has
 *	been received the frameCall routine is run (omitted if this is NULL).
 *	
 ...
 *	This also lowers the priority of the packet and frame signal handlers, which
 *	could delay the response to packet and frame interrupts. A better solution
 *	is needed which allows the signals to remain at high priority.
 *-
 */
	...
	)
{
    ...
    if (errorInit () == ERROR)
	{
		printErr ("sdsu_readTask: Failed to initialise error context structure.\n");       
		return;
	}
	
	/* Set up the sdsuReadContext taskVar and register our signal handler, sdsu_readSignal. */
	
	if (taskVarAdd(0, (int *) &sdsuReadContext) == ERROR)
	{
		ERROR_SET (0, "Failed to create sdsuReadContext task variable", ERROR_LOG_NOW);
		return;
	}
	...
	{
		/*
		 * Get the address of the next frame from the queue, transmitted by sdsu_readSignal.
		...
		{
			/* msgQReceive has returned ERROR or has received a message of the wrong size */
		...
		else if ( pFrame != NULL )
		{
			...
			while ((pFrame->header.status & SDSU_FSTAT_COMPLETE) == 0)
			{
				...
				if (packetCall != NULL)
				{
					(*packetCall) (sdsuReadContext, sdsuReadContext->appPrivate, pFrame);
				}
			}
			 ...
			 *
			 * NOTE: This is only a temporary solution. Reducing the priority here also
			 * reduces the priority of the signal handler, which could delay
			 * the response to frame interrupts.
			 */
			...
			if ( (pFrame->header.status & SDSU_FSTAT_ABORTED) == 0 )
			{
				...
				if (frameCall != NULL)
				{
					...
					(*frameCall) (sdsuReadContext, sdsuReadContext->appPrivate, pFrame);
			...
			/*
			 * We're done with this frame now. This call will return the frame to the free
			 * list unless the application code has reserved the frame with sdsuFrameReserve.
			 */

			if (sdsuFrameRelease (sdsuReadContext, pFrame) == ERROR)
			{
				ERROR_SET (0, "Problem releasing frame after readout", ERROR_LOG_NOW);
			}
		...
		{
			/*
			 * Something has gone wrong with the signal handler and it has reported an error
			 * by sending a NULL frame pointer.
			 */
 ...
 *
 *	DESCRIPTION:
 *	This is the task entry point for a simplified version of the readout task which handles all the
 *	image processing and synchronisation functions during the readout of the
 *	SDSU controller.
 *
 *	SEE ALSO:
	...
	)
{
	...
	while (TRUE)
	{
		...
		else
		{
			...
			{
				printf ("sdsu_simpleTask: Received message to process frame %p with %d frames\n",
			 ...
			 * Loop forever if the total number of frames is specified as 0 or negative.
			 * Terminate the loop if the frame status becomes non-zero (which means the
			 * SDSU controller has detected an error).
			 */

			sdsuReadContext->aborted = FALSE;
			sdsuReadContext->fatal = FALSE;
			failures = 0; 
			for (frame=0; (((pFrame->totalFrames <= 0) || (frame < pFrame->totalFrames)) &&
			     ...
			     frame++)
			{
				...
				/*
				 * A fast camera should wait for a synchronisation counting semaphore
				 * before the exposure starts.
				 */

				if ( sdsuReadContext->fastCamera )
				{
#ifdef DEBUG
					printf ("sdsu_simpleTask: Fast camera wait for sdsuSyncSem...\n");
#endif /* DEBUG */
					semTake ( sdsuSyncSem, WAIT_FOREVER );
				}

				/* Set the SDSU Frame Buffer Address to the address of the next frame. */
	
#ifdef DEBUG
				printf ("sdsu_simpleTask: Set FBA for frame %d\n", (frame+1));
#endif /* DEBUG */
				if (sdsuFrameSetFBA (sdsuReadContext, pFrame) == ERROR)
				{
					ERROR_SET (0, "Failed to set up frame buffer address for frame DMA",
						ERROR_LOG_NOW);
					sdsuReadContext->readStatus = SDSU_READ_ERROR;
...
#ifdef DEBUG
				printf ("sdsu_simpleTask: RDC\n");
				sdsuFrameShow ( pFrame );
#endif /* DEBUG */
				if ( !sdsuReadContext->fatal )
				{
					if (sdsuPrimitive (sdsuReadContext, "RDC", SDSU_IDENT_VME, NULL, NULL) == ERROR)
					{
						ERROR_SET (0, "Readout CCD (RDC) command failed", ERROR_LOG_NOW);
						sdsuReadContext->readStatus = SDSU_READ_ERROR;
						sdsuReadContext->fatal = TRUE;		/* Abandon the observation. */
					}
				}
				...
				if ( sdsuReadContext->fatal )
				{
					...
					if (sdsuPrimitive (sdsuReadContext, "ABT", SDSU_IDENT_VME, NULL, NULL) == ERROR )
					{
				...
				else if ( useInterrupts )
				{
					/*
					 * Using interrupts.
					 * Take the semaphore indicating the frame readout has completed.
					 * (Forget about packet interrupts for now).
					 *
					 * FUDGE - THE SEMTAKE IS PROGRAMMED TO TIME OUT AFTER frameTimeout ticks.
					 * THIS WILL ENABLE THE SYSTEM TO RECOVER FROM A LOST INTERRUPT.
					 * SMB - 16 Jan 1999.
					 */
				...
				else
				{
					 ...
					 * Poll for the correct number of packets in the data buffer.
					 * NOTE: THIS IS INEFFICIENT. IT WOULD BE BETTER TO PROCESS THE
					 * PREVIOUS FRAME BEFORE POLLING
					 * FOR THE END OF THIS FRAME.
					 */

					/* FUDGE: Wait for the expected exposure time before polling.	*/
					/* DO PROCESSING OF PREVIOUS FRAME HERE INSTEAD EVENTUALLY.		*/
#ifdef DEBUG
					printf ("sdsu_simpleTask: Waiting %d ticks until end of exposure...\n",
						sdsuReadContext->exposureTicks);
#endif /* DEBUG */
					taskDelay (sdsuReadContext->exposureTicks);

					/*
					 * A slow camera should grab a synchronisation counting semaphore
					 * now, when the readout is about to start. The task priority is
					 * temporarily increased to ensure the semaphore is taken and the
					 * fast camera is suspended as soon as possible.
					 */

					if ( !sdsuReadContext->fastCamera )
					{
#ifdef DEBUG
						printf ("sdsu_simpleTask: Slow camera wait for sdsuSyncSem...\n");
#endif /* DEBUG */
...
#ifdef DEBUG
					printf ("sdsu_simpleTask: Polling for packet count reaching %d...",
						context->packetsPerFrame);
#endif /* DEBUG */
					START_TIMEOUT (& context->timeoutStart);
					...
					{
						timeout = TIMEOUT_READ_DATA_SEC * context->packetsPerFrame;
					}

					while ( (pFrame->header.packetCount < context->packetsPerFrame) &&
			                (pFrame->header.status == 0) &&
							! (timeoutExpired (timeout, & context->timeoutStart))
					      )
					{
						taskDelay( DELAY_READ_DATA_TICKS );
					}


					/*
					 * If a timeout has occurred set the appropriate frame status bit
					 * and abort the observation (to reset the VME and timing boards
					 * for the next observation.
					 */

					if ( timeoutExpired (timeout, & context->timeoutStart) )
					{
						if (sdsuPrimitive (sdsuReadContext, "ABT", SDSU_IDENT_VME, NULL, NULL) == ERROR )
						{
							ERROR_SET (0, "Failed to abort observation after timeout",
								ERROR_LOG_NOW);
						}
				...
				/* Both cameras should now give back the synchronisation counting semaphore */

				semGive ( sdsuSyncSem );

				/*
				 * Fudge the frame number in the header so that detControl behaves correctly.
				 */

				pFrame->header.frameCount = pFrame->totalFrames - frame;

				/*
				 * Ensure the aborted bit is set in the frame header if the frame has been aborted.
				 */
				...
				if ( (pFrame->header.status & SDSU_FSTAT_OVERRUN) != 0 )
				{
					printf ("sdsu_simpleTask: Overrun error bit set in frame header\n");
				}
				if ( (pFrame->header.status & SDSU_FSTAT_CHECKSUM) != 0 )
				{
					printf ("sdsu_simpleTask: Checksum error bit set in frame header\n");
				}
				if ( (pFrame->header.status & SDSU_FSTAT_FRAMESYNC) != 0 )
				{
					printf ("sdsu_simpleTask: Framesync error bit set in frame header\n");
				}
				if ( (pFrame->header.status & SDSU_FSTAT_ABORTED) != 0 )
				{
					printf ("sdsu_simpleTask: Frame aborted bit set in frame header\n");
				}
				...
				/* If the observation has not been aborted, call the frame callback function. */
				...
				{
#ifdef DEBUG
					printf ("sdsu_simpleTask: No frame callback since frame aborted\n");
#endif /* DEBUG */
				}
			}
 ...
 *
 *	DESCRIPTION:
 *	The readout task is sent a SIGUSR1 signal by sdsu_frameSyncIsr.  On receipt of
 *	this the sdsu_readSignal routine flags the current frame as complete.  If there
 *	are further exposures to be read out, another frame is found, passed to the
 *	VME DSP and queued for the image processing to work on.
 *
 *	NOTE:
 *	This code executes in the context of the task executing sdsu_readTask, and therefore inherits
 *	that task's priority and task variables. This is how sdsuReadContext is shared.
 *
 *	SEE ALSO:
 *	sdsu_readTask
 *	
 ...
 *
 *	DEFICIENCIES:
 *	It is possible for the execution of the signal handler to be blocked if the CPU is busy executing
 *	a packet or frame callback at the time when a frame sync interrupt happens. Unfortunately, setting
 *	task priorities does not solve this problem since the signal handler inherits the same priority
 *	as the readout task.
 *
 *	If an error occurs in the signal handler it is difficult for the detector control task
 *	to discover that anything has gone wrong.
 *-
 */

void sdsu_readSignal
	(
	int sigNumber
	)
{
	SDSU_FRAME *	pFrame = sdsuReadContext->readFrame;	/* Current SDSU frame pointer.	*/
	...
	if (pFrame == NULL)
	{
		ERROR_SET (S_sdsuLib_INTERNAL_ERROR, "Frame sync signal received with no frame",
			ERROR_LOG_NOW);
		return;
	}
	 ...
	 * Ideally a frame should be grabbed immediately from the buffer, but if a frame is
	 * not available immediately the function will wait for SDSU_FRAME_FIND_TIMEOUT ticks
	 * until one becomes free. It is very important to keep synchronised with the
	 * SDSU controller, and any delay in finding a new frame here may cause the controller
	 * to lose synchronisation and throw away data. (The SDSU_FSTAT_OVERRUN bit and possibly
	 * also the SDSU_FSTAT_FRAMESYNC bit in the data header status word will be set if this happens).
	 */
	...
	{
		/* If a new frame could not be obtained, reuse the previous frame, which will
		 * result in data loss.
		 */
	...
	/*
	 * Set the frame buffer address (FBA) in the SDSU controller to match the address of
	 * the frame just found.
	 */

	if (sdsuFrameSetFBA (sdsuReadContext, pFrame) == ERROR)
	{
		ERROR_SET (0, "sdsu_readSignal: Failed to set up frame buffer address for next frame DMA",
			ERROR_LOG_NOW);
		sdsuFrameRelease (sdsuReadContext, pFrame);
		sdsuReadContext->readFrame = NULL;
		goto ERROR_EXIT;
	...
	if ( !skipFrame )
	{
		...
		{
			ERROR_SET (0, "sdsu_readSignal: Unable to send frame to message queue for processing",
				ERROR_LOG_NOW);
		}
	}
...
ERROR_EXIT:
	...
	if (msgQSend (sdsuReadContext->frameQueue, (char *) &pBadFrame, 
				 sizeof(SDSU_FRAME *), NO_WAIT, MSG_PRI_NORMAL) == ERROR)
	{
		ERROR_SET (0, "sdsu_readSignal: Unable to send bad frame to message queue for processing",
			ERROR_LOG_NOW);
		/* PROBLEM: WE ARE NOW STUFFED AND CAN'T TELL THE OUTSIDE WORLD AN ERROR HAS HAPPENED */
		return;
	}
	...
	)
{
	...
	{
		ERROR_SET (S_sdsuLib_READOUT_ACTIVE, "Readout task not idle", ERROR_LOG_SAVE);
		return (ERROR);
	}
	 ...
	 * Find a spare frame in the frame buffer.
	 * No timeout is specified because at the start of an observation all the frames should
	 * be available.
	 */
	...
	{
		ERROR_SET (0, "sdsuReadoutStart: Failed to get frame for first readout", ERROR_LOG_SAVE);
		return (ERROR);
	}

	/* Set the SDSU Frame Buffer Address to the address of the frame just obtained. */
	...
	{
		ERROR_SET (0, "Failed to set up frame buffer address for first frame DMA", ERROR_LOG_SAVE);
		sdsuFrameRelease(context, pFrame);
	...
	/*
	 * Send an initial message to the frame queue indicating the location of the frame buffer
	 * and the number of frames to be processed.
	 */
	...
	if (msgQSend (context->frameQueue, (char *) &pFrame, 
				 sizeof(SDSU_FRAME *), NO_WAIT, MSG_PRI_NORMAL) == ERROR)
	{
		ERROR_SET (0, "Unable to send frame to message queue for processing", ERROR_LOG_SAVE);
		return (ERROR);
	}
 ...
 *	period is that of the "ABT" entry of sdsuCmdTable, and is fixed.  No action is
 ...
 *
 *	DESCRIPTION:
 *	This routine writes the parameter paramValue to the address identified by the 
 *	paramName in the DSP identified by ID destId, which can take one of the
 ...
 *		SDSU_IDENT_UTL	=>	Refers to Utility DSP
 *
 *	The parameters for each of the three DSPs (VME, Timing and Utility) are defined
 *	by the symbols found in the object file which has been downloaded to the DSP.
 *	Parameter values are written using the WRM command, which means that this call
 *	*cannot* be used to communicate with the Timing or Utility DSPs during exposure/
 *	readout operation.
 *
 *	SEE ALSO:
 *	sdsuParamRead
	...
	)
{
	...
	{
		ERROR_SET1 (S_sdsuLib_INV_PARAM_NAME, "Parameter %s not found", ERROR_LOG_SAVE, paramName);
		return (ERROR);
	}
	...
	{
		ERROR_SET2 (0, "Failed to write %s parameter to DSP memory at address %#x", ERROR_LOG_SAVE,
			paramName, address);
		return (ERROR);
	}
 ...
 *
 *	DESCRIPTION:
 *	This routine writes the parameter paramValue to the Y address identified by the 
 *	paramName in the DSP identified by ID destId, which can take one of the
 *	values specified in the table below.  Note however that only the Timing DSP
 *	currently implements the WRP command, thus this routine cannot be used to send
 *	parameter updates to the other DSPs with the current DSP software.
 *
 *		SDSU_IDENT_VME	=>	Refers to VME DSP
 *		SDSU_IDENT_TIM	=>	Refers to Timing DSP
 *		SDSU_IDENT_UTL	=>	Refers to Utility DSP
 ...
 *	The parameters for each of the three DSPs (VME, Timing and Utility) are defined
 *	by the symbols found in the object file which has been downloaded to the DSP.
 *	Parameter values are written using the WRP command, which means that this call
 *	*can* be used to communicate with the Timing DSP during an exposure/readout
 *	operation, and after all parameter changes have been written an LDP command must
 *	be sent by the application to cause these changes to take effect.
 *
 *	SEE ALSO:
 *	sdsuParamRead
 *	sdsuParamWRP
	...
	)
{
	...
	if (symFindByNameAndType(context->paramSyms, paramName, (char **) &pAddressData[0], 
							 NULL, (SYM_TYPE) destId, (SYM_TYPE) ~0) == ERROR)
	{
		ERROR_SET1 (S_sdsuLib_INV_PARAM_NAME, "Parameter %s not found", ERROR_LOG_SAVE, paramName);
		return (ERROR);
	...
	{
		ERROR_SET1 (S_sdsuLib_INV_DSP_ADDRESS, "Parameter %s not in Y memory", ERROR_LOG_SAVE,
		 	paramName);
		return (ERROR);
	}
	...
	{
		ERROR_SET1 (0, "Failed to write %s parameter to DSP memory using WRP", ERROR_LOG_SAVE,
			paramName);
		return (ERROR);
	}
	...
	)
{
	...
	{
		ERROR_SET2 (0, "Failed to read %s parameter from DSP memory at address %#x", ERROR_LOG_SAVE,
			paramName, address);
		return (ERROR);
	}
 ...
 *
 *	DESCRIPTION:
 *	This routine prints out the value of the named parameter for a specified DSP,
 *	whose ID is specified in destId. Valid DSP IDs are listed in the table below.
 *
 *		SDSU_IDENT_HST	=>	Refers to the host CPU
 *		SDSU_IDENT_VME	=>	Refers to VME DSP
 *		SDSU_IDENT_TIM	=>	Refers to Timing DSP
 ...
 *	symbol addresses in the download file, and the value at this address is read
 *	from the DSP and printed.
 *
 *	SEE ALSO:
 *	sdsuParamRead
 *
 ...
 *	The parameter names and values are separated by white space. Each name/value
 *	pair is terminated by a newline. The parameters are directed to the DSP
 *	specified by destId, which can take one of the values specified
 *	in the table below.
 *
 ...
 *
 *	DEFICIENCIES:
 *	The function is not very flexible when it comes to parsing the parameter file.
 *	Comments can only occur at the beginning of the file and have to have a ";"
 *	in column 1. In-line comments are not currently supported. The remainder of
 *	the file after the comment block is assumed to consist of parameter name/value
 *	pairs only.
 *
 *	At present, files uploaded using this sdsuParamUpload cannot be downloaded
 *	without first editing the parameter file. Allowing in-line comments beginning
 *	with a ";" will solve this deficiency.
 *-
 */
	...
	)
{
	...
	if ((strstr (pParamFileName, ".par") == NULL) && (strstr (pParamFileName, ".PAR") == NULL))
	{
	...
	{
		ERROR_SET1 (0, "Failed to open parameter file, %s", ERROR_LOG_SAVE, pParamFileName);
		return (ERROR);
	}
	...
	while ( (nextChar = fgetc (pParamFile)) == ';' )		/* Test the next character for ';'	*/
	{
	...
	failures = 0;
	while ((nread = fscanf (pParamFile, "%s %s", pParamName, pParamString)) != EOF)
	{
		if ( nread == 2 )
		{

			/*
			 * If the DSP destination, destId, is specified as -1, extract the destination
			 * DSP identifier from first 2 characters of the parameter name; otherwise use the
			 * DSP identifier specified. (Specifying -1 allows each parameter from a single file
			 * to be individually directed to the appropriate DSP).
			 */
			...
			/*
			 * If the parameter value begins with "0x" or "$" assume it is a hexadecimal integer,
			 * otherwise assume it is decimal integer.
			 */
			...
			if ( (pHexString = strstr (pParamString, "0x")) == NULL )
			{
				...
				{
					/* The string contains neither "0x" nor "$". Assume it is decimal. */
				...
				{
					/* The string contains "$". Assume a hexadecimal value starts at pHexString+1. */
			...
			{
				ERROR_SET1 (0, "Error setting %s parameter", ERROR_LOG_SAVE, pParamName);
				fclose (pParamFile);
				return (ERROR);
			}
		}
		...
		{
			ERROR_SET2 (0, "Problem reading parameter file at line containing %s %s",
	...
	if ( failures > 0 )
	{
		ERROR_SET2 (0, "%d conversion failures occurred reading parameter file %s", ERROR_LOG_SAVE,
	...
	MESSAGE_LOG1 (MSG_MINDEBUG, "Downloaded parameters from %s OK.", pParamFileName);
 ...
 *	(>)	sourceId	(const long)	ID of DSP from which parameters will be uploaded
 *
 *	FUNCTION VALUE:
 *	(STATUS)  OK, or ERROR if an error occured while uploading.
 *
 ...
 *
 *	DEFICIENCIES:
 *	Only parameters known to this function will be uploaded. That means this function
 *	has to be kept up to date with ICD 1.6/1.10.
 *
	...
	)
{
	...
	if ( (sourceId == SDSU_IDENT_VME) || (sourceId == -1) )
	{
		...
		{
			if (sdsuParamRead (context, SDSU_IDENT_VME, vmeParamNames[i], &paramValue) == ERROR)
			{
				errorClear();						/* Ignore any error */
				fprintf (pParamFile, "%12s\t******** \t;\tFailed to read parameter\n",
				         vmeParamNames[i]);
			}
			else
			{
	...
	if ( (sourceId == SDSU_IDENT_TIM) || (sourceId == -1) )
	{
		...
		{
			if (sdsuParamRead (context, SDSU_IDENT_TIM, timParamNames[i], &paramValue) == ERROR)
			{
				errorClear();						/* Ignore any error */
				fprintf (pParamFile, "%12s\t******** \t;\tFailed to read parameter\n",
				         timParamNames[i]);
	...
	if ( (sourceId == SDSU_IDENT_UTL) || (sourceId == -1) )
	{
		...
		{
			if (sdsuParamRead (context, SDSU_IDENT_UTL, utlParamNames[i], &paramValue) == ERROR)
			{
				errorClear();						/* Ignore any error */
				fprintf (pParamFile, "%12s\t******** \t;\tFailed to read parameter\n",
				         utlParamNames[i]);
 ...
 *
 *	DESCRIPTION:
 *	This routine reads and prints out the values of all the known SDSU status parameters, translating
 *	the contents of those parameters into a status report.
 *
 *	SEE ALSO:
 *	sdsuParamRead
 *
 ...
 *
 *	DEFICIENCIES:
 *	This function makes assumptions about the names of the SDSU status parameters
 *	described in ICD 1.6/1.10 and needs to be changed each time the names of those
 *	parameters change.
 *-
 */

STATUS	sdsuStatusShow
	(
	SDSU_ID			context
	)
{
	...
	/*
	 * Initialise the return value. An attempt will be made to display every status parameter even if
	 * failures occur, so the return value is remembered until the end.
	 */
	...
	{
		ERROR_SET (0, ERROR_MSG_NONE, ERROR_LOG_NOW);	/* Force error message to be displayed. */
		returnValue = ERROR;
	}
	else
	{
 ...
 *
 *	DESCRIPTION:
 *	This routine reads and prints out the values of all the known SDSU temperature parameters.
 *	The chip temperature is sampled several times and the mean value calculated.
 *
 *	SEE ALSO:
 *	sdsuParamRead
 *
 ...
 *	parameters change.
 *
 *	The coefficients used in the calculations within this function have been copied
 *	from a similar function written by Tim Hardy. These do not seem to correspond to
 *	the coefficients documented in ICD 1.6/1.10, which I find confusing.
 *	SMB - 28 Jan 1999.
 *-
 */
...
{
	uint32	dataOut [] = {0x000000, 0xffffff, 0x555555, 0xaaaaaa, 0x5c5c5c, 0x123456};
	uint32	dataIn;
	int		i;
	...
	for (i = 0; i < NELEMENTS (dataOut); i++)					/* Do TDL for each test pattern		*/
	{
		if (verbose) printf ("Test pattern %#x\n", dataOut [i]);

		if (sdsuPrimitive (context, "TDL", destId, & dataOut [i], & dataIn) == ERROR)
		{
		...
		/* The data mis-match test is not necessary if the controller is being simulated. */

		if ((!context->simulate) && (dataOut [i] != dataIn))
		{
			ERROR_SET3 (S_sdsuLib_TEST_FAIL,
				"TDL data mis-match, wrote %#x but read %#x from DSP %d", ERROR_LOG_SAVE,
				dataOut [i], dataIn, destId);
			return (ERROR);
		}
	...
	)
{
	...
	switch (destId)
	{
		...
		/*
		 * Test the ability to execute an RDM command at the specified address in each of the
		 * DSP's memory spaces. Each DSP is handled in exactly the same way.
		 */
		...
		case SDSU_IDENT_TIM:
			...
			* pMemSpace = SDSU_MEM_SPACE_X;
			...
			{
				ERROR_SET2 (0, "RDM failed at start of X space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pAddress = address;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at end of X space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pMemSpace = SDSU_MEM_SPACE_Y;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pAddress = address;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pMemSpace = SDSU_MEM_SPACE_P;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at start of P space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pAddress = address;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at end of P space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pMemSpace = SDSU_MEM_SPACE_E;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at start of E space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pAddress = address;
			if (sdsuPrimitive (context, "RDM", destId, & address, & data) == ERROR)
			{
				ERROR_SET2 (0, "RDM failed at end of E space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			break;
 ...
 *	check that the same value is obtained. Finally it will restore the old value.
 *	The pointers pMemSpace and pAddress are written with the memory-space and
 *	address being tested at any instant so that, in the event of an error, they
 *	can be examined by the calling routine in order to determine the cause of the
 *	failure.
 *
 *	EXTERNAL VARIABLES:
 *	None
 ...
 *	and has been commented out pending further investigation. The test was failing
 *	even with correctly working SDSU hardware. SMB - 14 October 1998.
 *-
 */
	...
	)
{
	...
	switch (destId)
	{
		...
		/*
		 * Test the ability to execute an WRM command at the specified address in the
		 * DSP's Y memory space. Each DSP is handled in exactly the same way.
		 */
		...
		case SDSU_IDENT_TIM:
			...
			* pMemSpace = SDSU_MEM_SPACE_Y;
			...
			if (sdsuPrimitive (context, "RDM", destId, & address, & oldData) == ERROR)
			{
				ERROR_SET2 (0, "RDM 1 failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			newData = ~oldData;
			if (sdsuPrimitive (context, "WRM", destId, & address, & newData) == ERROR)
			{
				ERROR_SET2 (0, "WRM 1 failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			if (sdsuPrimitive (context, "RDM", destId, & address, & readBackData) == ERROR)
			{
				ERROR_SET2 (0, "RDM 2 failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
...
/*
			...
			{
				ERROR_SET2 (S_sdsuLib_TEST_FAIL, "Wrote %lu, Read back %lu", ERROR_LOG_SAVE,
					newData, readBackData);
				ERROR_SET2 (0,
					"Read back test failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
*/

			if (sdsuPrimitive (context, "WRM", destId, & address, & oldData) == ERROR)
			{
				ERROR_SET2 (0, "WRM 2 failed at start of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			...
			* pAddress = address;
			...
			if (sdsuPrimitive (context, "RDM", destId, & address, & oldData) == ERROR)
			{
				ERROR_SET2 (0, "RDM 1 failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			newData = ~oldData;
			if (sdsuPrimitive (context, "WRM", destId, & address, & newData) == ERROR)
			{
				ERROR_SET2 (0, "WRM 1 failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			if (sdsuPrimitive (context, "RDM", destId, & address, & readBackData) == ERROR)
			{
				ERROR_SET2 (0, "RDM 2 failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
/* Read back test commented out - see "DEFICIENCIES" section above */
/*
			if ( (!context->simulate) && (readBackData != newData) )
			{
				ERROR_SET2 (S_sdsuLib_TEST_FAIL, "Wrote %lu, Read back %lu", ERROR_LOG_SAVE,
					newData, readBackData);
				ERROR_SET2 (0,
					"read back test failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
*/
			if (sdsuPrimitive (context, "WRM", destId, & address, & oldData) == ERROR)
			{
				ERROR_SET2 (0, "WRM 2 failed at end of Y space (%#x) for DSP %d", ERROR_LOG_SAVE,
					(* pAddress | * pMemSpace), destId);
				return (ERROR);
			}
			break;
	...
	)
{
	...
	{
		ERROR_SET2 (S_sdsuLib_INV_PROC_ID, "Invalid source (%lu) and target (%lu) IDs", ERROR_LOG_SAVE,
			sourceId, destId);
		return (ERROR);
	}

	(context->pCmdBuffer) [0] = sdsu_makeHeader (pCmdDef, sourceId, destId, TRUE);
																	/* Get the cmd-header word	*/
	(context->pCmdBuffer) [1] = sdsu_makeCmdWord (pCmdDef);			/* ... and the cmd mnemonic	*/
	...
	{
		ERROR_SET (S_sdsuLib_INV_CMD_ARG_COUNT, "Too many command arguments", ERROR_LOG_SAVE);
		return (ERROR);
	}

	for (argIndex = 0; argIndex < pCmdDef->cmdArgCount; argIndex++)	/* Copy any arguments		*/
		(context->pCmdBuffer) [2 + argIndex] = pCmdArg [argIndex];

	/*
	 * Following the command header word, the command-word and any arguments there may
	 * be unused words in the command buffer. Fill these with a recognisable value to
	 * make them readily identifiable for debugging purposes (e.g. when examining the
	 * contents of context->pCmdBuffer[]).
	 */

	for (argIndex = pCmdDef->cmdArgCount; argIndex < CMD_BUF_NWORD - 2; argIndex++)
	{
 ...
 *
 *	DESCRIPTION:
 *	This routine generates a header word for an SDSU command or reply (if cmdHeader
 *	is TRUE, a command header is generated, otherwise a reply header is generated).
 *
 *	EXTERNAL VARIABLES:
 *	None
 *
...
)
{
	...
	{
		nWord += pCmdDef->cmdArgCount;					/* if a command, get the number of arguments*/
	}
	else												/* ... or is it a reply header word ?		*/
	{
		...
		{
			/*
			 * A reply for which the reply type is BIT_FIELD_REPLY_NONE cannot, by definition, have a
			 * reply-header word. This is a serious error condition.
			 */

			ERROR_SET (S_sdsuLib_INTERNAL_ERROR, "BIT_FIELD_REPLY_NONE cannot have reply-header word",
				ERROR_LOG_SAVE);
			return (0);
		}
		else
		{
			nWord += pCmdDef->repArgCount;				/* if a reply, get the number of arguments	*/
		}
	}

	return (sourceId << 16 | destId << 8 | nWord);	/* Assemble and return the reply header */
}
 ...
 *
 *	DESCRIPTION:
 *	This routine tests whether the reply read from an SDSU controller in response
 *	to a previously-issued command is valid. In order to be valid, the reply
 *	must be one of the values defined in the definition of the SDSU command.
 *
 *	EXTERNAL VARIABLES:
 *	(>)	sdsuSymTab		(SYMTAB_ID)		Symbol table ID.
 *
	...
	)
{
	...
	/*
	 * Check that the reply is one of those identified in the bit-field (pRepDef->repBitMask) which
	 * defines valid replies to this command. Note that this routine is only called if a reply is
	 * expected for the command, so that pRepDef->repBitMask should always be non-zero in this
	 * routine.
	 */
...
{
	if		(sdsu_areStringsSame (pField, OMF_FIELD_START))		return (OMF_FIELD_IDENT_START);
	else if	(sdsu_areStringsSame (pField, OMF_FIELD_END))		return (OMF_FIELD_IDENT_END);
	else if	(sdsu_areStringsSame (pField, OMF_FIELD_DATA))		return (OMF_FIELD_IDENT_DATA);
	else if	(sdsu_areStringsSame (pField, OMF_FIELD_BLOCKDATA))	return (OMF_FIELD_IDENT_BLOCKDATA);
	else if	(sdsu_areStringsSame (pField, OMF_FIELD_SYMBOL))	return (OMF_FIELD_IDENT_SYMBOL);
	else if	(sdsu_areStringsSame (pField, OMF_FIELD_COMMENT))	return (OMF_FIELD_IDENT_COMMENT);
	else
	{
		ERROR_SET1 (S_sdsuLib_INV_OMF_REC_TYPE, "Invalid OMF record-type, %s", ERROR_LOG_SAVE,
			pField);
	...
	)
{
	...
	for (i = 0; i < strlen (pString); i++)
	{
		...
		else if	((nibble >= 0x41) && (nibble <= 0x46))	hexResult |= nibble - 0x37;
		else if	((nibble >= 0x61) && (nibble <= 0x66))	hexResult |= nibble - 0x57;
	}
	return (hexResult);
}
 ...
 *
 *	FUNCTION VALUE:
 *	(uint32)	version number (0xffffffff if a version number could not be extracted)
 *
 *	PURPOSE:
 *	Convert a CVS version string into numeric version number
 *
 *	DESCRIPTION:
 *	This routine interprets pString as a CVS version number aa[.bb[.cc[.dd]]] and 
 *	returns a 4-byte integer numeric equivalent, with each set of digits in their
 *	own byte starting at the MSB.  It scans pString for the first digit character,
 *	then converts the first series of digits into the top byte.  Any digits after
 *	a '.' are converted into the next byte and so on.
 ...
 *
 *	PURPOSE:
 *	Check the Reply buffer for unread words - caused perhaps by a system reset or
 *	timeout from a previous command. If detected, flag an error and read all unread
 *	words from the buffer
 *
 *	DESCRIPTION:
 *	This routine reads the reply buffer at the current read address. If an unread word 
 *	is detected: an error is logged, the buffer printed, then cleared, then printed
 *	again. Any system reset detected will cause an additional error to be logged.
 *
 *
 *	EXTERNAL VARIABLES:
 *	None
...
STATUS	sdsu_checkRepBuf (SDSU_ID context)
{
	...
	if (sdsuFullDebug)
		...
		if ((context->pRepBuffer [context->repBufCounter] & 0xff000000)	!= 0)
		{
			...
			ERROR_SET (S_sdsuLib_REPLY_BUFFER_ERROR, "Unexpected data found in Reply buffer",
				ERROR_LOG_SAVE);
			...
			while ((context->pRepBuffer [context->repBufCounter] & 0xff000000) != 0)
			{
					...
					== SDSU_STRING_TO_UINT ("SYR"))
				{
					...
					ERROR_SET (S_sdsuLib_REPLY_BUFFER_ERROR, "Unexpected System reset detected",
						ERROR_LOG_SAVE);
				}
 ...
 *
 *	DESCRIPTION:
 *	This routine increments the pointer in the local SDSU reply buffer. This buffer
 *	gets written by the SDSU VME DSP, and the DSP maintains its own copy of the buffer
 *	pointer and increments this pointer as it issues replies to the VME CPU. The
 *	local VME CPU (the thing running sdsuLib under VxWorks) is responsible for keeping
 *	the reply-buffer pointer in synchronisation with that maintained by the VME DSP.
 *	Parameter nIncrement gives the number of words by which the pointer should be
 *	incremented. The pointer wraps around to the start of the buffer every REP_BUF_NWORD
 *	words.
 *
 *	EXTERNAL VARIABLES:
 *	None
 ...
 *
 *	DEFICIENCIES:
 *	It is possible for the DSP and host to lose synchronisation and end up with different
 *	reply buffer pointers. Care should be taken to call this function to step over any
 *	new data in the reply buffer.
 *-
 */
	...
	)
{
	...
	for (i = 0; i < nIncrement; i++)
	{
		...
		if (cacheFlush (DATA_CACHE, & (context->pRepBuffer [context->repBufCounter]), 4) == ERROR)
		{
			ERROR_SET (0, "Cache flush for reply buffer failed", ERROR_LOG_SAVE);		
			return (ERROR);
		}
 ...
 *
 *	DESCRIPTION:
 *	This routine extracts the address of a previously-allocated SDSU reply buffer from
 *	the context structure, convert it to a VME bus address (i.e. to the addres to which the
 *	VME DSP should write its replies), and then issues an SRA command to the VME DSP in
 *	order to set this reply address.
 *
 *	EXTERNAL VARIABLES:
 *	None
 *
	...
	)
{
	...
	pCmdArg [0] = ((uint32) pHostAddress >> 16) & 0xffff;	/* Get top 16 bits of address			*/
	pCmdArg [1] = (uint32) pHostAddress & 0xffff;			/* ... and bottom 16 bits of  address	*/
	...
	{
		context->repBufCounter = oldRepBufCounter;			/* Restore counter if SRA failed		*/
	...
	)
{
	...
	switch ((int) destId)
	{
		...
		/* Check that the address is within the defined range for one of the memory spaces */
		...
		case SDSU_IDENT_TIM:
			...
			{
				ERROR_SET1 (S_sdsuLib_INV_DSP_ADDRESS, "Invalid DSP address, %#x", ERROR_LOG_SAVE,
					address);
				valid = FALSE;
			}
			break;
		...
		case SDSU_IDENT_HST:
			ERROR_SET (S_sdsuLib_INV_DSP_ADDRESS, "Host specified as DSP", ERROR_LOG_SAVE);
			valid = FALSE;
			break;

		default:
			ERROR_SET1 (S_sdsuLib_INV_DSP_ADDRESS, "Unknown DSP ID, %lu", ERROR_LOG_SAVE, destId);
			valid = FALSE;
	}
	return (valid);
}
