	...
	)
{
	...
	for (fieldNumber = 0; fieldNumber < pCaContext->nField; fieldNumber++)
	{
		 ...
		 * contain the string "ag:wfs:".
		 */

		if ( strstr( ca_name(pCaContext->pChannelId [fieldNumber]), "ag:wfs:") == NULL )
		{
 ...
 *	"ag:wfs:status" with the macro TOP = "ag:wfs:" can legitimately be referred to
 *	via pRecordName) as either "ag:wfs:status" or "status".
 *
