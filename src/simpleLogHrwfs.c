 ...
 * 0. wfsHrwfsDb.h contains the top level record prefix, TOP, and cicsLib.h
   ...
   )
{
   ...
   int            pLogFd [NUM_FILES]; /* File descriptor array.               */
   ...
   {
      ERROR_SET1 (0, "Invalid number of processors on VME bus, %d", 
                  ERROR_LOG_NOW, nProcOnBus);
      return (ERROR);
   }
      ...
      for (procNumber = 0; procNumber < nProcOnBus; procNumber++)
      {
         if (FD_ISSET (pLogFd [procNumber], & readFds))
         {
            ...
            if ( (nByte > 0) && (nByte <= ERROR_LOG_BUFFER_SIZE) )
            {
               ...
               /*
                * Log the message to the console. The errorConsoleSem semaphore 
                * is used to prevent messages from different tasks overlapping 
                * on the console.
                */
                ...
                * Increment the message counter and every nFlush messages close 
                * and reopen the file.
                */
                   ...
                   == 0)
               {
                  ...
                  if (cicsDbPut (TOP LOGTASK_ERROR_LOG_NAME0, pMessage, 
                  ...
                  /*
                   * If the message is longer than EPICS_MAX_BYTES_STRING_ATTRIB
                   * characters copy the next part of the message into the 
                   * history log overflow record.
                   */
               ...
               else
               {
                  ...
                  strncpy (pHistoryMessage, pMsgBuffer, 
                           EPICS_MAX_BYTES_STRING_ATTRIB);
                  if (cicsDbPut (TOP LOGTASK_HISTORY_LOG_NAME0, pMessage, 
                                 DBF_STRING, (void *) pHistoryMessage)
                      == ERROR)
                  {
                     ERROR_SET (S_errorLog_EPICS_ERROR, pMessage, 
                   ...
                   * EPICS_MAX_BYTES_STRING_ATTRIB characters copy the next 
                   * part of the message into the history log overflow record.
                   */
                  ...
                  if ( strlen(pMsgBuffer) > EPICS_MAX_BYTES_STRING_ATTRIB )
                  {
                     ...
                     strncpy (pHistoryMessage, pSubString, 
                              EPICS_MAX_BYTES_STRING_ATTRIB);
                     if (cicsDbPut (TOP LOGTASK_HISTORY_LOG_NAME1, pMessage, 
                                    DBF_STRING, (void *) pHistoryMessage)
                         == ERROR)
                     {
                        ERROR_SET (S_errorLog_EPICS_ERROR, pMessage, 
            ...
            else
            {
                ...
                * or equal to zero or greater than the buffer size. Something 
                * has gone wrong.
                */
