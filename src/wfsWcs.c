 ...
 *
 * PURPOSE:
 * Wavefront Sensing WCS functions
 *
 * DESCRIPTION:
 * This library contains a miscellaneous collection of WCS functions.
 *
 * FUNCTION NAME(S):
 * wfsUpdateAstCtx - Updates local WCS context
 *
 ...
 * including epToVxLib.h and dbDefs.h at the same time - TO BE RESOLVED 
 * LATER. SMB - 16 November 1998.
 *
 *INDENT-OFF*
...
/*
 * Store the current TCS tracking variables in global variables. 
 * It is safe to do this because all wavefront sensors will 
 * share this information. The global variables are initialised 
 ...
 *
 *   PURPOSE:
 *   Initialise update local WCS context genSub record
 *
 *   DESCRIPTION:
 *   This function carries out any initialisation required by the update 
 *   local WCS context genSub record. In practise it just initialises the 
 *   error library.
 *
 *   REFERENCE:
 *
 ...
 *
 *   DESCRIPTION:
 *   This function updates the current WCS context using information obtained 
 *   from the TCS. It is assumed that the INPA field of the genSub record with 
 *   which this function is associated is connected to the VALA field of the 
 *   "astCtx" record in the TCS database. It is assumed that the NOA field is 
 *   defined as AST_CTXA_SIZE (=39) and the FTA field is defined as DOUBLE.
 *
 *   REFERENCE:
   ...
   )
{
    ...
    * prevent the output of a "TCS database not connected" error followed 
   ...
   /*
    * Pass the information contained in field A to astSetCtx to set the local 
    * WCS context.
    */
