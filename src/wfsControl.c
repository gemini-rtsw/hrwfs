static struct {void *v; char *c;} rcsid = {&rcsid,
   "$Id: wfsControl.c,v 1.2 2000-01-05 20:10:03 cboyer Exp $"};
 ...
 *
 *   DESCRIPTION:
 *   This file contains the function "wfsControl", which is executed by the 
 *   wavefront sensing control task; together with any private functions used 
 *   by this task alone.
 *   The "wfsControl" task implements some, but not all, of the systemwide 
 *   wavefront sensing control commands. Commands not implemented here (e.g. 
 *   "test") are implemented instead using Capfast.
 *
 *   INCLUDE FILES:
 *   wfsLib.h
 *   wfsControl.h
 ...
 *
 *   DEVELOPMENT NOTE:
 *   Perhaps the "init" command should be implemented in SNL, so the 
 *   "INITIALIZING" state can remain until all the WFSs have finished 
 *   initialising. SMB - 18 Oct 1998.
 *
 *   AUTHORS:
 *   Nick Dillon
 *   Steven Beard
 ...
 * Changed default debug mode to FULL (for as long as we are carrying out 
 * engineering tests)
 *
 ...
 * sysextProcNumGet replaced with sysProcNumGet. initStat changed from 
 * int to long
 *
 * Revision 1.3  1998/06/30 12:42:10  smb
 * Dependency on sysextLib and mpPipeDrv can be removed using NO_SYSEXTLIB and 
 * NO_MPPIPEDRV macros.
 *
...
STATUS   wfsControl (void)
{
   ...
   if (epToVxRecContextGet (WFS_CONTROL_INIT_SIR_NAME, & pInitContext, NULL) 
       == ERROR)
   {
   ...
   if (epToVxRecContextGet (WFS_CONTROL_MEAS_SIR_NAME, & pMeasContext, NULL) 
       == ERROR)
   {
   ...
   while (! wfsControlStop)
   {
       ...
       * until a command becomes available. The wfsControl task is aborted if 
      ...
      else if (commandNumber == WFS_CONTROL_CMD_INIT)
      {
          ...
          * controllers (by the Capfast code) so simply reset the WFS 
      ...
      else if (commandNumber == WFS_CONTROL_CMD_STARTMEASURE)
      {
         ...
         measState = TRUE;
         if (epToVxPipeWrite( NULL, (char *) &measState, pMeasContext ) == 
             ERROR)
         {
      ...
      else if (commandNumber == WFS_CONTROL_CMD_CALIBRATE)
      {
         ...
         MESSAGE_LOG (MSG_LOG, 
         "Start calibration observation command - TO BE IMPLEMENTED");
      }
